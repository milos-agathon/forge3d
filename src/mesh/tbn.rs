...
impl TbnData {
    ...
    pub fn is_valid(&self) -> bool {
        ...
        let computed_handedness = if cross.dot(self.normal) >= 0.0 { 1.0 } else { -1.0 };
...
impl Triangle {
    fn new(vertices: &[TbnVertex], indices: [u32; 3]) -> Self {
        ...
        Self { indices, positions, normals, uvs }
    }
...
pub fn generate_tbn(vertices: &[TbnVertex], indices: &[u32]) -> Vec<TbnData> {
    ...
    for triangle_indices in indices.chunks_exact(3) {
        let tri_indices = [triangle_indices[0], triangle_indices[1], triangle_indices[2]];
    ...
    for (i, vertex) in vertices.iter().enumerate() {
        ...
        let final_tangent = if orthogonal_tangent.length_squared() < 1e-6 {
            ...
            let up = if normal.y.abs() > 0.9 { Vec3::X } else { Vec3::Y };
...
pub fn generate_cube_tbn() -> (Vec<TbnVertex>, Vec<u32>, Vec<TbnData>) {
    let vertices = vec![
        // Front face
        TbnVertex { position: Vec3::new(-1.0, -1.0, 1.0), normal: Vec3::Z, uv: Vec2::new(0.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, -1.0, 1.0), normal: Vec3::Z, uv: Vec2::new(1.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, 1.0, 1.0), normal: Vec3::Z, uv: Vec2::new(1.0, 1.0) },
        TbnVertex { position: Vec3::new(-1.0, 1.0, 1.0), normal: Vec3::Z, uv: Vec2::new(0.0, 1.0) },
        
        // Back face
        TbnVertex { position: Vec3::new(1.0, -1.0, -1.0), normal: Vec3::NEG_Z, uv: Vec2::new(0.0, 0.0) },
        TbnVertex { position: Vec3::new(-1.0, -1.0, -1.0), normal: Vec3::NEG_Z, uv: Vec2::new(1.0, 0.0) },
        TbnVertex { position: Vec3::new(-1.0, 1.0, -1.0), normal: Vec3::NEG_Z, uv: Vec2::new(1.0, 1.0) },
        TbnVertex { position: Vec3::new(1.0, 1.0, -1.0), normal: Vec3::NEG_Z, uv: Vec2::new(0.0, 1.0) },
        
        // Left face
        TbnVertex { position: Vec3::new(-1.0, -1.0, -1.0), normal: Vec3::NEG_X, uv: Vec2::new(0.0, 0.0) },
        TbnVertex { position: Vec3::new(-1.0, -1.0, 1.0), normal: Vec3::NEG_X, uv: Vec2::new(1.0, 0.0) },
        TbnVertex { position: Vec3::new(-1.0, 1.0, 1.0), normal: Vec3::NEG_X, uv: Vec2::new(1.0, 1.0) },
        TbnVertex { position: Vec3::new(-1.0, 1.0, -1.0), normal: Vec3::NEG_X, uv: Vec2::new(0.0, 1.0) },
        
        // Right face
        TbnVertex { position: Vec3::new(1.0, -1.0, 1.0), normal: Vec3::X, uv: Vec2::new(0.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, -1.0, -1.0), normal: Vec3::X, uv: Vec2::new(1.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, 1.0, -1.0), normal: Vec3::X, uv: Vec2::new(1.0, 1.0) },
        TbnVertex { position: Vec3::new(1.0, 1.0, 1.0), normal: Vec3::X, uv: Vec2::new(0.0, 1.0) },
        
        // Bottom face
        TbnVertex { position: Vec3::new(-1.0, -1.0, -1.0), normal: Vec3::NEG_Y, uv: Vec2::new(0.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, -1.0, -1.0), normal: Vec3::NEG_Y, uv: Vec2::new(1.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, -1.0, 1.0), normal: Vec3::NEG_Y, uv: Vec2::new(1.0, 1.0) },
        TbnVertex { position: Vec3::new(-1.0, -1.0, 1.0), normal: Vec3::NEG_Y, uv: Vec2::new(0.0, 1.0) },
        
        // Top face
        TbnVertex { position: Vec3::new(-1.0, 1.0, 1.0), normal: Vec3::Y, uv: Vec2::new(0.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, 1.0, 1.0), normal: Vec3::Y, uv: Vec2::new(1.0, 0.0) },
        TbnVertex { position: Vec3::new(1.0, 1.0, -1.0), normal: Vec3::Y, uv: Vec2::new(1.0, 1.0) },
        TbnVertex { position: Vec3::new(-1.0, 1.0, -1.0), normal: Vec3::Y, uv: Vec2::new(0.0, 1.0) },
    ];
...
mod tests {
    ...
    fn test_plane_tbn_generation() {
        ...
        for tbn in &tbn_data {
            assert!((tbn.normal - Vec3::Y).length() < 1e-3, "Normal should be Y-up");
        }
    }
