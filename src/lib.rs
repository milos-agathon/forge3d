...
#[cfg(any(
    feature = "enable-normal-mapping",
    feature = "enable-pbr",
    feature = "enable-ibl",
    feature = "enable-csm"
))]
pub mod pipeline;
mod transforms; // Advanced rendering pipelines
...
fn _pt_render_gpu(
    ...
    if let Ok(list) = scene.downcast::<PyList>() {
        for item in list.iter() {
            ...
            let center_any = d
                .get_item("center")?
                .ok_or_else(|| PyValueError::new_err("sphere.center missing"))?;
            let radius_any = d
                .get_item("radius")?
                .ok_or_else(|| PyValueError::new_err("sphere.radius missing"))?;
            let albedo_any = d
                .get_item("albedo")?
                .ok_or_else(|| PyValueError::new_err("sphere.albedo missing"))?;
    ...
    let origin: (f32, f32, f32) = cdict
        .get_item("origin")?
        .and_then(|v| v.extract().ok())
        .unwrap_or((0.0, 0.0, 0.0));
    let look_at: (f32, f32, f32) = cdict
        .get_item("look_at")?
        .and_then(|v| v.extract().ok())
        .unwrap_or((0.0, 0.0, -1.0));
    let up: (f32, f32, f32) = cdict
        .get_item("up")?
        .and_then(|v| v.extract().ok())
    ...
    let fwd = {
        let fx = look_at.0 - origin.0;
        let fy = look_at.1 - origin.1;
        let fz = look_at.2 - origin.2;
    ...
    let rlen = (right[0] * right[0] + right[1] * right[1] + right[2] * right[2])
        .sqrt()
        .max(1e-6);
    ...
    let rgba =
        crate::path_tracing::compute::PathTracerGPU::render(width, height, &spheres, uniforms)
...
fn _pt_render_aovs_gpu(
    ...
    if let Ok(seq) = aovs.downcast::<PyList>() {
        for item in seq.iter() {
            ...
            if let Some(k) = AovKind::from_name(&name) {
                want.push(k);
            }
        }
    }
    ...
    let map = crate::path_tracing::compute::PathTracerGPU::render_aovs(
        width, height, &spheres, uniforms, aov_mask,
    )
        ...
        if let Some(buf) = map.get(&k) {
            match k {
                ...
                | AovKind::Emission => {
                    ...
                    let arr = unsafe {
                        PyArray3::<f32>::new(py, [height as usize, width as usize, 3], false)
                    };
                    let dst = unsafe { arr.as_slice_mut()? };
                ...
                AovKind::Depth => {
                    ...
                    let arr = unsafe {
                        PyArray2::<f32>::new(py, [height as usize, width as usize], false)
                    };
                    let dst = unsafe { arr.as_slice_mut()? };
                    let floats: &[f32] = unsafe {
                        std::slice::from_raw_parts(
                ...
                AovKind::Visibility => {
                    ...
                    let arr = unsafe {
                        PyArray2::<u8>::new(py, [height as usize, width as usize], false)
                    };
                    let dst = unsafe { arr.as_slice_mut()? };
...
impl Renderer {
    ...
    #[pyo3(text_signature = "($self)")]
    pub fn render_triangle_rgba<'py>(
        &mut self,
        py: Python<'py>,
        ...
        if need > self.readback_size {
            ...
            registry
                .check_budget(need)
                .map_err(|e| RenderError::device(e))?;
        ...
        let arr3 = Array3::from_shape_vec((self.height as usize, self.width as usize, 4), pixels)
    ...
    pub fn add_terrain(
        ...
        let as_f32: Result<(Vec<f32>, usize, usize), pyo3::PyErr> = (|| {
            ...
            if !ro32.is_c_contiguous() {
                return Err(pyo3::exceptions::PyRuntimeError::new_err(
                    "heightmap must be C-contiguous (row-major)",
                ));
            }
        ...
        let (heights, width, height) = match as_f32 {
            Ok(ok) => ok,
            Err(_) => {
                let arr64: &PyArray2<f64> = heightmap.downcast().map_err(|_| {
    ...
    pub fn terrain_stats(&self) -> pyo3::PyResult<(f32, f32, f32, f32)> {
        let terr = self.terrain.as_ref().ok_or_else(|| {
            pyo3::exceptions::PyRuntimeError::new_err(
    ...
    pub fn normalize_terrain(
        &mut self,
        mode: &str,
        range: Option<(f32, f32)>,
        eps: Option<f32>,
    ) -> pyo3::PyResult<()> {
        let terr = self.terrain.as_mut().ok_or_else(|| {
            pyo3::exceptions::PyRuntimeError::new_err(
                "no terrain uploaded; call add_terrain() first",
            )
        })?;
    ...
    pub fn upload_height_r32f(&mut self, py: Python<'_>, heights: Option<&PyAny>) -> PyResult<()> {
        ...
        if bytes > limit {
            return Err(pyo3::exceptions::PyMemoryError::new_err(format!(
                "height texture {}x{} exceeds 512 MiB ({} bytes)",
                w, h, bytes
            )));
        }
    ...
    pub fn debug_read_height_patch<'py>(
        &mut self,
        py: Python<'py>,
        x: u32,
        y: u32,
    ...
    #[pyo3(text_signature = "($self)")]
    pub fn read_full_height_texture<'py>(
        &mut self,
        py: Python<'py>,
    ) -> pyo3::PyResult<Bound<'py, PyArray2<f32>>> {
        let terr = self.terrain.as_ref().ok_or_else(|| {
    ...
    #[pyo3(text_signature = "($self)")]
    pub fn render_triangle_rgba_with_ptr<'py>(
        &mut self,
        py: Python<'py>,
    ) -> PyResult<(Bound<'py, PyArray3<u8>>, usize)> {
        let g = ctx();
        self.render_into_offscreen(g)?;
    ...
    pub fn set_sun(
        ...
        if !azimuth_deg.is_finite() || !elevation_deg.is_finite() {
            return Err(pyo3::exceptions::PyValueError::new_err(
                "azimuth and elevation must be finite",
            ));
        }
        ...
        if self.postfx_enabled {
            ...
            if let Some(chain) = self.postfx_chain.as_mut() {
                if !chain.list_enabled_effects().is_empty() {
                    ...
                    let hdr_out_view =
                        hdr_out_tex.create_view(&wgpu::TextureViewDescriptor::default());
                    chain
                        .execute_chain(&g.device, &mut encoder, &hdr_view, &hdr_out_view, None)
    ...
    fn ensure_terrain_infrastructure(&mut self, g: &crate::gpu::GpuContext) -> PyResult<()> {
        ...
        let (colormap_lut, individual_luts) = if terrain_pipeline.supports_descriptor_indexing() {
            ...
            for palette_type in &palette_types {
                let (lut, _format) =
                    terrain::ColormapLUT::new(&g.device, &g.queue, &g.adapter, *palette_type)
...
fn device_probe(py: Python<'_>, backend: Option<String>) -> PyResult<PyObject> {
    ...
    let adapter = match adapter {
        ...
        None => {
            ...
            dict.set_item("millis", t0.elapsed().as_secs_f64() * 1000.0)
                .ok();
    ...
    dict.set_item("device_type", devtype_str(info.device_type))
        .ok();
    dict.set_item("vendor_id", info.vendor as u32).ok();
    dict.set_item("device_id", info.device as u32).ok();
    dict.set_item("features", format!("{:?}", adapter.features()))
        .ok();
    dict.set_item("limits", format!("{:?}", adapter.limits()))
        .ok();
    ...
    let (_device, _queue) = match pollster::block_on(adapter.request_device(
        ...
        Err(e) => {
            ...
            dict.set_item("message", format!("request_device failed: {}", e))
                .ok();
            dict.set_item("millis", t0.elapsed().as_secs_f64() * 1000.0)
                .ok();
            return Ok(dict.into_any().unbind());
...
#[cfg(feature = "terrain_spike")]
pub use terrain::{Globals, TerrainUniforms};
...
fn png_to_numpy(py: Python<'_>, path: PathBuf) -> PyResult<Bound<'_, PyArray3<u8>>> {
    let img = image::open(&path).map_err(|e| {
        pyo3::exceptions::PyRuntimeError::new_err(format!("failed to open PNG: {e}"))
    })?;
...
fn numpy_to_png(_py: Python<'_>, path: PathBuf, array: &PyAny) -> PyResult<()> {
    ...
    if let Ok(arr3) = array.downcast::<PyArray3<u8>>() {
        ...
        if shape.len() != 3 {
            return Err(pyo3::exceptions::PyRuntimeError::new_err(
                "expected 3-D array of shape (H,W,3) or (H,W,4)",
            ));
        }
        let (h, w, c) = (shape[0], shape[1], shape[2]);
        let v = ro
            .as_slice()
            .map_err(|_| pyo3::exceptions::PyRuntimeError::new_err("array not contiguous"))?;
        match c {
            4 => {
                let img = image::RgbaImage::from_raw(w as u32, h as u32, v.to_vec()).ok_or_else(
                    || pyo3::exceptions::PyRuntimeError::new_err("invalid RGBA buffer"),
                )?;
                img.save(&path)
                    .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
                return Ok(());
            ...
            3 => {
                let img =
                    image::RgbImage::from_raw(w as u32, h as u32, v.to_vec()).ok_or_else(|| {
                        pyo3::exceptions::PyRuntimeError::new_err("invalid RGB buffer")
                    })?;
                img.save(&path)
                    .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
                return Ok(());
            ...
            _ => {
                return Err(pyo3::exceptions::PyRuntimeError::new_err(
                    "expected last dimension to be 3 (RGB) or 4 (RGBA)",
                ));
            }
        }
    }
    ...
    if let Ok(arr2) = array.downcast::<PyArray2<u8>>() {
        ...
        if shape.len() != 2 {
            return Err(pyo3::exceptions::PyRuntimeError::new_err(
                "expected 2-D array of shape (H,W) for grayscale",
            ));
        }
    ...
    Err(pyo3::exceptions::PyRuntimeError::new_err(
        "unsupported array; expected uint8 (H,W), (H,W,3) or (H,W,4)",
    ))
}
...
fn c5_build_framegraph_report(py: Python<'_>) -> PyResult<PyObject> {
    use crate::core::framegraph_impl::types::{PassType, ResourceDesc, ResourceType};
...
fn mesh_generate_cube_tbn(py: Python<'_>) -> PyResult<PyObject> {
    ...
    let py_vertices = PyList::new_bound(
        py,
        vertices.iter().map(|v| {
            let vertex_dict = PyDict::new_bound(py);
            vertex_dict
                .set_item("position", (v.position.x, v.position.y, v.position.z))
                .unwrap();
            vertex_dict
                .set_item("normal", (v.normal.x, v.normal.y, v.normal.z))
                .unwrap();
    ...
    let py_tbn = PyList::new_bound(
        py,
        tbn_data.iter().map(|tbn| {
            let tbn_dict = PyDict::new_bound(py);
            tbn_dict
                .set_item("tangent", (tbn.tangent.x, tbn.tangent.y, tbn.tangent.z))
                .unwrap();
            tbn_dict
                .set_item(
                    "bitangent",
                    (tbn.bitangent.x, tbn.bitangent.y, tbn.bitangent.z),
                )
                .unwrap();
            tbn_dict
                .set_item("normal", (tbn.normal.x, tbn.normal.y, tbn.normal.z))
                .unwrap();
