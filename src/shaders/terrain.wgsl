...
fn vs_main(v: VSIn) -> VSOut {
  ...
  out.clip = UBO.proj * UBO.view * world;

  // Normalize height to ~[0,1]
  let hr = max(UBO.spacing_h_exag_pad.y, 1e-6);
  let h  = v.pos.y * UBO.spacing_h_exag_pad.z; // exaggeration
  let h_norm = 0.5 + 0.5 * (h / (0.5 * hr));
  out.height = clamp(h_norm, 0.0, 1.0);

  // Simple N·L
  let L = normalize(UBO.sun_exposure.xyz);
...
fn fs_main(inp: VSOut) -> @location(0) vec4<f32> {
  ...
  var col = textureSample(colormap_tex, colormap_samp, uv).rgb;

  // Cheap diffuse to avoid flat images in tests
  let lit = mix(0.15, 1.0, inp.ndotl) * col;

  let exposure = max(UBO.sun_exposure.w, 1e-3);
