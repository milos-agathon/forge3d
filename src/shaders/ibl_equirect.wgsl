...
fn direction_to_equirect(dir: vec3<f32>) -> vec2<f32> {
    ...
    let u = det_div(det_atan2(d.z, d.x), TWO_PI) + 0.5;
    let v = det_div(det_acos(clamp(d.y, -1.0, 1.0)), PI);
...
fn cs_equirect_to_cubemap(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let pixel = det_div2(vec2<f32>(f32(gid.x) + 0.5, f32(gid.y) + 0.5), vec2<f32>(f32(size)));
