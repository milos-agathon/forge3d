...
fn brdf_toon(normal: vec3<f32>, view: vec3<f32>, light: vec3<f32>, base_color: vec3<f32>, params: ShadingParamsGPU) -> vec3<f32> {
    ...
    let threshold = det_mix(0.4, 0.9, 1.0 - params.roughness);
    if (n_dot_l > threshold) {
        return base_color;
    }
    let rim = det_pow(1.0 - saturate(det_dot3(view, normal)), 2.0);
    let rim_color = det_mix3(vec3<f32>(0.0), base_color, params.sheen);
