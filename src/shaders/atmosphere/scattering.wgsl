...
fn phase_rayleigh(cos_theta: f32) -> f32 {
    ...
    return det_div(3.0 * (1.0 + det_barrier(c * c)), 16.0 * AETHER_PI);
}

fn phase_mie(cos_theta: f32, g_raw: f32) -> f32 {
    let c = clamp(cos_theta, -1.0, 1.0);
    let g = clamp(g_raw, -0.999, 0.999);
    let gg = det_barrier(g * g);
    return det_div(det_barrier(3.0 * (1.0 - gg)) * (1.0 + det_barrier(c * c)), det_barrier(8.0 * AETHER_PI * (2.0 + gg))
            * det_pow(max(det_barrier(1.0 + gg) - det_barrier(det_barrier(2.0 * g) * c), 1.0e-6), 1.5));
}
...
fn atmosphere_load_2d_linear(tex: texture_2d<f32>, uv_raw: vec2<f32>) -> vec4<f32> {
    ...
    let f = fract(p);
    let a = det_mix4(textureLoad(tex, lo, 0), textureLoad(tex, vec2<i32>(hi.x, lo.y), 0), f.x);
    let b = det_mix4(textureLoad(tex, vec2<i32>(lo.x, hi.y), 0), textureLoad(tex, hi, 0), f.x);
    return det_mix4(a, b, f.y);
}
...
fn atmosphere_distance_to_boundary(altitude_m: f32, mu: f32, params: AtmosphereScatteringUniforms) -> f32 {
    ...
    let radius = det_barrier(bottom + clamp(altitude_m, 0.0, top - bottom));
    let top_disc = det_barrier(det_barrier(det_barrier(radius * radius) * mu) * mu) + det_barrier((top - radius) * (top + radius));
    let top_distance = max(det_barrier(-radius * mu) + det_sqrt(max(top_disc, 0.0)), 0.0);
    let ground_disc = det_barrier(det_barrier(det_barrier(radius * radius) * mu) * mu)
        - det_barrier((radius - bottom) * (radius + bottom));
    if (mu < 0.0 && ground_disc >= 0.0) {
        return max(det_barrier(-radius * mu) - det_sqrt(ground_disc), 0.0);
    }
    return top_distance;
}
...
fn atmosphere_ray_hits_ground(altitude_m: f32, mu: f32,
    ...
    let ground_disc = det_barrier(det_barrier(det_barrier(radius * radius) * mu) * mu)
        - det_barrier((radius - bottom) * (radius + bottom));
    return ground_disc >= 0.0 && det_barrier(-radius * mu) - det_sqrt(max(ground_disc, 0.0)) >= 0.0;
}
...
fn sample_transmittance(
    ...
    let uv = vec2<f32>(0.5 * (clamp(zenith_cosine, -1.0, 1.0) + 1.0),
        clamp(det_div(altitude_m, atmosphere_height), 0.0, 1.0));
    ...
    return det_pow3(boundary_t, vec3<f32>(clamp(det_div(distance_m, boundary_distance), 0.0, 1.0)));
}
...
fn sky_radiance(view_dir_y_up: vec3<f32>, sun_dir_y_up: vec3<f32>,
    ...
    let view = det_normalize3(view_dir_y_up); let sun = det_normalize3(sun_dir_y_up);
    return sample_inscatter(multiple_scatter_lut, camera_altitude_m, view.y,
        sun.y, det_dot3(view,sun), params);
}

fn aether_sky_radiance(view_dir_raw: vec3<f32>) -> vec3<f32> {
    let view=det_normalize3(view_dir_raw); let sun=det_normalize3(atmosphere.sun_direction_intensity.xyz);
    ...
    if (sun_visible && alignment>=det_cos(sun_radius)) {
        ...
        radiance = det_barrier3(radiance + det_barrier3(det_barrier3(det_barrier3(boundary_t*sun_intensity)
            *det_smoothstep(det_cos(sun_radius),1.0,alignment))*40.0));
    }
...
fn cs_render_aether_sky(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let clip=vec4<f32>(det_barrier(uv.x*2.0)-1.0,1.0-det_barrier(uv.y*2.0),1.0,1.0);
    let view_pos=det_mat4_mul_vec4(camera.inv_proj, clip);
    let view_vs=det_normalize3(det_div3(view_pos.xyz, vec3<f32>(view_pos.w)));
    let view_ws=det_normalize3((det_mat4_mul_vec4(camera.inv_view, vec4<f32>(view_vs,0.0))).xyz);
