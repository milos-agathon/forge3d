...
use wgpu::{Device, Queue, Buffer, ComputePipeline, BindGroup, BufferUsages};
...
impl GpuBvhBuilder {
    ...
    pub fn new(device: Arc<Device>, queue: Arc<Queue>) -> Result<Self> {
        ...
        let sort_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/radix_sort_pairs.wgsl").into()),
        });
        ...
        let sort_count_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let sort_scatter_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let link_nodes_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let init_leaves_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let refit_leaves_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let refit_internal_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let refit_iterative_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
    ...
    pub fn build(&mut self, triangles: &[Triangle], options: &BuildOptions) -> Result<BvhHandle> {
        ...
        if triangles.len() > (1 << 20) {
            anyhow::bail!("Triangle count {} exceeds maximum of 1M triangles", triangles.len());
        }
        ...
        if estimated_memory > 512 * 1024 * 1024 {
            anyhow::bail!("Estimated GPU memory usage {}MB exceeds 512MB budget", estimated_memory / (1024 * 1024));
        }
    ...
    pub fn refit(&mut self, handle: &mut BvhHandle, triangles: &[Triangle]) -> Result<()> {
        ...
        if triangles.len() as u32 != gpu_data.primitive_count {
            anyhow::bail!("Triangle count mismatch: expected {}, got {}", 
                         gpu_data.primitive_count, triangles.len());
        }
        ...
        self.execute_refit(&aabb_buffer, &gpu_data.nodes_buffer, gpu_data.primitive_count)?;
    ...
    fn estimate_memory_usage(&self, prim_count: u32) -> Result<u64> {
        ...
        let total = centroids_size + morton_codes_size + indices_size + 
                   nodes_size + aabbs_size + sort_temp_size + histogram_size;
...
impl GpuBvhBuilder {
    fn create_buffers(&self, prim_count: u32, centroids: &[[f32; 3]], aabbs: &[Aabb]) -> Result<GpuBuffers> {
    ...
    fn generate_morton_codes(&self, buffers: &GpuBuffers, world_aabb: &Aabb, prim_count: u32) -> Result<()> {
    ...
    fn build_bvh_topology(&self, buffers: &GpuBuffers, prim_count: u32, node_count: u32) -> Result<()> {
        // Implementation would dispatch BVH linking kernels
        Ok(())
    }
    
    fn execute_refit(&self, aabb_buffer: &Buffer, nodes_buffer: &Buffer, prim_count: u32) -> Result<()> {
