    ...
    pub fn test_aabb(&self, aabb: &AABB) -> bool {
        ...
        for plane in &self.planes {
            ...
            for corner in &corners {
                let distance = plane.x * corner.x + plane.y * corner.y + plane.z * corner.z + plane.w;
                if distance >= 0.0 {
                    inside = true;
                    break;
                }
...
impl BatchManager {
    ...
    pub fn generate_batches(&mut self) -> Result<Vec<Batch>, RenderError> {
        ...
        self.stats.total_primitives = self.polygon_data.len() + self.line_data.len() + self.point_data.len();
    ...
    fn batch_polygons(&self, visible: Vec<(VectorId, PolygonDef, AABB)>) -> Vec<Batch> {
        self.create_batches(
            visible.into_iter().map(|(id, _, aabb)| (id, aabb)).collect(),
