...
fn parse_header<R: BufRead>(reader: &mut R) -> RenderResult<(u32, u32)> {
    ...
    if !line.starts_with("#?RADIANCE") && !line.starts_with("#?RGBE") {
        return Err(RenderError::upload("Invalid HDR file: missing magic header".to_string()));
    }
    ...
    while reader.read_line(&mut line)
        .map_err(|e| RenderError::io(format!("Failed to read HDR header: {}", e)))? > 0 {
        ...
        if line_trimmed.starts_with("FORMAT=") {
            if line_trimmed != "FORMAT=32-bit_rle_rgbe" && line_trimmed != "FORMAT=32-bit_rle_xyze" {
                return Err(RenderError::upload(format!("Unsupported HDR format: {}", line_trimmed)));
            }
            format_found = true;
        }
    ...
    if !format_found {
        return Err(RenderError::upload("HDR file missing FORMAT specification".to_string()));
    }
    ...
    if parts.len() != 4 {
        return Err(RenderError::upload(format!("Invalid HDR resolution line: {}", resolution_line)));
    }
    
    let height = parts[1].parse::<u32>()
        .map_err(|_| RenderError::upload(format!("Invalid HDR height: {}", parts[1])))?;
    let width = parts[3].parse::<u32>()
        .map_err(|_| RenderError::upload(format!("Invalid HDR width: {}", parts[3])))?;
    
    if width == 0 || height == 0 {
        return Err(RenderError::upload("HDR image dimensions cannot be zero".to_string()));
    }
...
fn read_scanline<R: Read>(reader: &mut R, width: u32, y: u32) -> RenderResult<Vec<[u8; 4]>> {
    ...
    reader.read_exact(&mut header)
        .map_err(|e| RenderError::io(format!("Failed to read scanline header at row {}: {}", y, e)))?;
    
    // Check if this is a new-style RLE scanline
    if header[0] == 2 && header[1] == 2 && header[2] == (((width >> 8) & 0xFF) as u8) && header[3] == ((width & 0xFF) as u8) {
        // New-style RLE
        ...
        for pixel in scanline.iter_mut().skip(1) {
            reader.read_exact(pixel)
                .map_err(|e| RenderError::io(format!("Failed to read pixel data at row {}: {}", y, e)))?;
        }
        ...
        while pos < width as usize {
            ...
            if run_length > 128 {
                ...
                if pos + repeat_count > width as usize {
                    return Err(RenderError::upload("HDR RLE run exceeds scanline width".to_string()));
                }
                
                let mut value = [0u8; 1];
                reader.read_exact(&mut value)
                    .map_err(|e| RenderError::io(format!("Failed to read RLE repeat value: {}", e)))?;
                ...
                if pos + copy_count > width as usize {
                    return Err(RenderError::upload("HDR literal run exceeds scanline width".to_string()));
                }
                
                for i in 0..copy_count {
                    let mut value = [0u8; 1];
                    reader.read_exact(&mut value)
                        .map_err(|e| RenderError::io(format!("Failed to read literal value: {}", e)))?;
                    scanline[pos + i][component] = value[0];
                }
