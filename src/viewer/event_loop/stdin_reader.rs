...
pub fn parse_stdin_command(l: &str) -> Option<Vec<ViewerCmd>> {
    ...
    if l.starts_with(":sky ") || l == ":sky" || l.starts_with("sky ") {
        if let Some(arg) = l.split_whitespace().nth(1) {
            match arg {
                ...
                "preetham" => {
                    return Some(vec![ViewerCmd::SkyToggle(true), ViewerCmd::SkySetModel(0)])
                }
                "hosek-wilkie" | "hosekwilkie" | "hosek" | "hw" => {
                    return Some(vec![ViewerCmd::SkyToggle(true), ViewerCmd::SkySetModel(1)])
                }
                _ => println!(
    ...
    if l.starts_with(":oit") || l.starts_with("oit ") {
        ...
        if toks.len() >= 2 {
            ...
            return Some(vec![ViewerCmd::SetOitEnabled {
                enabled,
                mode: mode_str,
            }]);
        } else {
