...
impl Viewer {
    ...
    pub(crate) fn selected_frame_camera(&self) -> FrameCamera {
        ...
        if let Some(point_cloud) = self
            .point_cloud
            .as_ref()
            .filter(|point_cloud| point_cloud.point_count > 0)
        {
    ...
    pub(crate) fn prepare_frame_anchor(&mut self) {
        ...
        let (point_count, source_bytes, render_cache_bytes, gpu_instance_bytes) = self
            .point_cloud
            .as_ref()
            .map(|point_cloud| {
        ...
        let taa_enabled = self
            ...
            .map(crate::viewer::terrain::ViewerTerrainScene::is_taa_enabled)
            .unwrap_or_else(|| {
                self.taa_renderer
                    .as_ref()
                    .is_some_and(crate::core::taa::TaaRenderer::is_enabled)
            });
        let (ssgi_enabled, ssgi_temporal_enabled, ssr_effect_enabled) = self
            .gi
            .as_ref()
            .map(|gi| {
    ...
    fn refresh_after_rebase(&mut self, frame: FrameCamera) {
        if let Some(point_cloud) = self.point_cloud.as_mut() {
            if let Err(err) = point_cloud.repack_for_anchor(&self.queue, &frame.anchor) {
                eprintln!("[viewer] point-cloud rebase refresh failed: {err}");
            }
        }
