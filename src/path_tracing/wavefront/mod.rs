...
use wgpu::{Device, Queue, CommandEncoder, Buffer, BufferUsages, BindGroup};
use std::sync::Arc;
...
impl WavefrontScheduler {
    ...
    pub fn render_frame(
        ...
        self.dispatch_raygen(&mut encoder, uniforms_buffer, scene_bind_group, accum_bind_group)?;
        ...
        for iteration in 0..max_iterations {
            ...
            let ray_count = self.queue_buffers.get_active_ray_count(&self.device, &self.queue, &mut encoder)?;
            if ray_count == 0 {
                break;
            }
            ...
            self.dispatch_shade(&mut encoder, uniforms_buffer, scene_bind_group, accum_bind_group)?;
    ...
    fn dispatch_intersect(
        ...
        let uniforms_bind_group = self.create_uniforms_bind_group(uniforms_buffer)?;
        let queue_bind_group = self.queue_buffers.create_intersect_bind_group(&self.device)?;
    ...
    fn dispatch_compact(&self, encoder: &mut CommandEncoder) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn create_uniforms_bind_group(&self, uniforms_buffer: &Buffer) -> Result<BindGroup, Box<dyn std::error::Error>> {
