...
impl MaterialShading {
    ...
    pub fn lambert(roughness: f32) -> Self {
        Self { brdf: BrdfModel::Lambert.as_u32(), roughness, ..Default::default() }
    }

    pub fn phong(roughness: f32, metallic: f32) -> Self {
        Self { brdf: BrdfModel::Phong.as_u32(), roughness, metallic, ..Default::default() }
    }

    pub fn disney(roughness: f32, metallic: f32, sheen: f32, clearcoat: f32, subsurface: f32) -> Self {
        Self {
    ...
    pub fn anisotropic(brdf_model: BrdfModel, roughness: f32, anisotropy: f32) -> Self {
        Self { brdf: brdf_model.as_u32(), roughness, anisotropy, ..Default::default() }
    }
}
