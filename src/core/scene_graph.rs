...
impl SceneGraph {
    ...
    pub fn add_child(&mut self, parent_id: NodeId, child_id: NodeId) -> RenderResult<()> {
        ...
        if !self.nodes.contains_key(&parent_id) {
            return Err(RenderError::render(&format!("Parent node {:?} not found", parent_id)));
        }
        if !self.nodes.contains_key(&child_id) {
            return Err(RenderError::render(&format!("Child node {:?} not found", child_id)));
        }
        
        // Check for circular dependency
        if self.would_create_cycle(parent_id, child_id)? {
            return Err(RenderError::render("Adding child would create circular dependency"));
        }
    ...
    pub fn traverse<V: SceneVisitor>(&mut self, visitor: &mut V) -> Result<(), Box<dyn std::error::Error>> {
        // Update transforms first
        self.update_transforms().map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    ...
    fn collect_descendants(&self, node_id: NodeId, descendants: &mut Vec<NodeId>) -> RenderResult<()> {
    ...
    fn update_node_transforms(&mut self, node_id: NodeId) -> RenderResult<()> {
        ...
        let (local_transform, world_dirty, children) = {
            if let Some(node) = self.nodes.get(&node_id) {
                (node.local_transform.clone(), node.world_dirty, node.children.clone())
            } else {
    ...
    fn traverse_node<V: SceneVisitor>(&mut self, node_id: NodeId, visitor: &mut V) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Enter the node
        if let Some(node) = self.nodes.get(&node_id) {
            visitor.enter_node(node, &world_matrix).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
        }
        ...
        // Exit the node
        if let Some(node) = self.nodes.get(&node_id) {
            visitor.exit_node(node).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
        }
...
mod tests {
    ...
    fn test_scene_graph_transform_inheritance() {
        ...
        // Set transforms
        graph.get_node_mut(root).unwrap().local_transform.set_translation(Vec3::new(1.0, 0.0, 0.0));
        graph.get_node_mut(child).unwrap().local_transform.set_translation(Vec3::new(0.0, 1.0, 0.0));
        ...
        assert_eq!(root_node.world_matrix.w_axis.xyz(), Vec3::new(1.0, 0.0, 0.0));
        
        // Child should have combined transform (parent + local)
        assert_eq!(child_node.world_matrix.w_axis.xyz(), Vec3::new(1.0, 1.0, 0.0));
    }
