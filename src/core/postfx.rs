...
/// Bloom effect: bright-pass, separable blur, and composite
pub struct BloomEffect {
    config: PostFxConfig,
    // Layouts and pipelines for the three stages
    layout_bright: BindGroupLayout,
    layout_blur: BindGroupLayout,
    pipe_bright: ComputePipeline,
    pipe_blur_h: ComputePipeline,
    pipe_blur_v: ComputePipeline,
    sampler: Sampler,
    ping_pong_pair: Option<usize>,
}
...
impl BloomEffect {
    pub fn new(device: &Device) -> RenderResult<Self> {
        ...
        // Blur layout (sampler + src + dst)
        let layout_blur = device.create_bind_group_layout(&BindGroupLayoutDescriptor {
        ...
        // Pipelines
        let bright_mod = device.create_shader_module(ShaderModuleDescriptor {
            label: Some("bloom_brightpass"),
            source: ShaderSource::Wgsl(include_str!("../shaders/bloom_brightpass.wgsl").into()),
        });
        let blur_h_mod = device.create_shader_module(ShaderModuleDescriptor {
            label: Some("bloom_blur_h"),
            source: ShaderSource::Wgsl(include_str!("../shaders/bloom_blur_h.wgsl").into()),
        });
        let blur_v_mod = device.create_shader_module(ShaderModuleDescriptor {
            label: Some("bloom_blur_v"),
            source: ShaderSource::Wgsl(include_str!("../shaders/bloom_blur_v.wgsl").into()),
        });

        let pipe_bright = device.create_compute_pipeline(&ComputePipelineDescriptor {
            label: Some("bloom_bright_pipeline"),
            layout: Some(&device.create_pipeline_layout(&PipelineLayoutDescriptor {
                label: Some("bloom_bright_layout"),
                bind_group_layouts: &[&layout_bright],
        ...
        let pipe_blur_h = device.create_compute_pipeline(&ComputePipelineDescriptor {
            label: Some("bloom_blur_h_pipeline"),
            layout: Some(&device.create_pipeline_layout(&PipelineLayoutDescriptor {
                label: Some("bloom_blur_h_layout"),
                bind_group_layouts: &[&layout_blur],
                push_constant_ranges: &[],
            })),
            module: &blur_h_mod,
            entry_point: "main",
        });
        let pipe_blur_v = device.create_compute_pipeline(&ComputePipelineDescriptor {
            label: Some("bloom_blur_v_pipeline"),
            layout: Some(&device.create_pipeline_layout(&PipelineLayoutDescriptor {
                label: Some("bloom_blur_v_layout"),
                bind_group_layouts: &[&layout_blur],
                push_constant_ranges: &[],
            })),
            module: &blur_v_mod,
...
impl PostFxEffect for BloomEffect {
    ...
    fn execute(
        ...
        let h = resource_pool.height.max(1);
        let gx8 = (w + 7) / 8;
        let gy8 = (h + 7) / 8;
        ...
        // Bright-pass: input -> A
        let bg_bright = device.create_bind_group(&BindGroupDescriptor {
        ...
        // Blur H: A -> B
        let bg_blur_h = device.create_bind_group(&BindGroupDescriptor {
        ...
        // Blur V: B -> output (acts as composite target; additive composite omitted in scaffold)
        let bg_blur_v = device.create_bind_group(&BindGroupDescriptor {
...
impl PostFxEffect for SimpleBlurEffect {
    ...
    fn execute(
        ...
        // Placeholder no-op to keep build stable; real compute dispatch integrated under feature gates.
        Ok(())
    }
}
