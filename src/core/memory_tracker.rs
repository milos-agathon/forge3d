...
use wgpu::{BufferUsages, TextureFormat, Device, Buffer, BufferDescriptor};
    ...
    fn allocate_block(&mut self) -> Option<(u64, u64, Arc<Mutex<u32>>)> {
        ...
        for i in 0..self.free_blocks.len() {
            ...
            if size >= self.allocation_size {
                ...
                self.allocated_blocks.insert(offset, (self.allocation_size, ref_count.clone()));
...
impl MemoryPoolManager {
    ...
    pub fn allocate_bucket(&mut self, size: u32) -> Result<PoolBlock, String> {
        ...
        let bucket_index = self.size_buckets
            .iter()
        ...
        if let Some((id, offset, ref_count)) = self.pools[bucket_index].allocate_block() {
            ...
            Err(format!("Failed to allocate {} bytes from pool bucket {}", size, bucket_index))
        }
    }
    ...
    pub fn defragment(&mut self) -> DefragStats {
        ...
        let frag_before: f32 = self.pools.iter()
            .map(|pool| pool.fragmentation_ratio())
            .sum::<f32>() / self.pools.len() as f32;
        ...
        let frag_after: f32 = self.pools.iter()
            .map(|pool| pool.fragmentation_ratio())
            .sum::<f32>() / self.pools.len() as f32;
    ...
    pub fn get_stats(&mut self) -> MemoryPoolStats {
        ...
        let total_frag: f32 = self.pools.iter()
            .map(|pool| pool.fragmentation_ratio())
            .sum::<f32>();
        ...
        self.stats.largest_free_block = self.pools
            .iter()
...
static GLOBAL_POOL_MANAGER: std::sync::OnceLock<Arc<Mutex<MemoryPoolManager>>> = std::sync::OnceLock::new();
...
pub fn init_global_pools(device: &Device) -> Arc<Mutex<MemoryPoolManager>> {
    GLOBAL_POOL_MANAGER.get_or_init(|| {
        Arc::new(Mutex::new(MemoryPoolManager::new(device)))
    }).clone()
}
