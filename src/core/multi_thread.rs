...
pub trait CommandTask: Send + Sync {
    ...
    fn execute(&self, encoder: &mut CommandEncoder, device: &Device, queue: &Queue) -> Result<usize, Self::Error>;
    ...
    fn drop(&mut self) {
        ...
        for worker in &mut self.workers {
            if let Some(handle) = worker.handle.take() {
                handle.join().unwrap_or_else(|_| eprintln!("Worker thread panicked"));
            }
        }
    }
}
...
impl MultiThreadRecorder {
    ...
    pub fn record_parallel<T: CommandTask + 'static>(
        ...
        self.initialize_pool().map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send>)?;
        
        let start_time = if self.config.enable_profiling { Some(Instant::now()) } else { None };
        
        // Channel for collecting results
        let (result_sender, result_receiver) = mpsc::channel::<Result<ThreadCommandBuffer, Box<dyn std::error::Error + Send>>>();
        ...
        for (task_index, task) in tasks.into_iter().enumerate() {
            ...
            let job = move || {
                let result = Self::record_task_on_thread(
                    task,
                    device,
                    queue,
                    config,
            ...
            if let Some(ref pool) = self.thread_pool {
                pool.execute(job).map_err(|_| Box::new(std::io::Error::new(
        ...
        if let Some(start) = start_time {
            ...
            println!("Multi-thread recording completed in {}ms with {} tasks", 
                    total_time, results.len());
        }
    ...
    pub fn record_and_submit<T: CommandTask + 'static>(
        ...
        self.submit_buffers(buffers).map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send>)
    }
    ...
    fn record_task_on_thread<T: CommandTask>(
        ...
        let start_time = if config.enable_profiling { Some(Instant::now()) } else { None };
        
        // Create command encoder
        let mut encoder = device.create_command_encoder(&CommandEncoderDescriptor {
            label: Some(&format!("{}_{}_thread_{:?}", config.label_prefix, task_index, thread_id)),
        });
        
        // Execute the task
        let command_count = task.execute(&mut encoder, &device, &queue)
...
mod tests {
    ...
    impl MockTask {
        fn new(name: String, work_duration_ms: u64, counter: Arc<AtomicUsize>) -> Self {
            Self { name, work_duration_ms, counter }
        }
    }
    ...
    impl CommandTask for MockTask {
        ...
        fn execute(&self, _encoder: &mut CommandEncoder, _device: &Device, _queue: &Queue) -> Result<usize, Self::Error> {
            // Simulate some work
