...
impl MatrixStack {
    ...
    pub fn push(&mut self) -> RenderResult<()> {
        if self.stack.len() >= self.max_depth {
            return Err(RenderError::render(
                &format!("Matrix stack overflow: maximum depth {} exceeded", self.max_depth)
            ));
        }
    ...
    pub fn transform(&mut self, translation: glam::Vec3, rotation: glam::Quat, scale: glam::Vec3) {
        self.mult(Mat4::from_scale_rotation_translation(scale, rotation, translation));
    }
    ...
    pub fn set_max_depth(&mut self, max_depth: usize) -> RenderResult<()> {
        ...
        if self.stack.len() > new_max {
            return Err(RenderError::render(
                &format!("Cannot set max depth to {}: current stack depth is {}", 
                        new_max, self.stack.len())
            ));
        }
