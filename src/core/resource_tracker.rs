...
impl Drop for ResourceHandle {
    fn drop(&mut self) {
        ...
        match self {
            ResourceHandle::Buffer { size, is_host_visible } => {
                tracker.free_buffer_allocation(*size, *is_host_visible);
            }
            ResourceHandle::Texture { width, height, format } => {
...
mod tests {
    ...
    fn test_register_buffer_helper() {
        ...
        
        {
            ...
            assert_eq!(after_alloc_metrics.buffer_count, initial_metrics.buffer_count + 1);
            assert_eq!(after_alloc_metrics.buffer_bytes, initial_metrics.buffer_bytes + 2048);
            assert_eq!(after_alloc_metrics.host_visible_bytes, initial_metrics.host_visible_bytes + 2048);
        }
        ...
        assert_eq!(final_metrics.host_visible_bytes, initial_metrics.host_visible_bytes);
    }
    ...
    fn test_register_texture_helper() {
        ...
        
        {
            ...
            assert_eq!(after_alloc_metrics.texture_count, initial_metrics.texture_count + 1);
            assert_eq!(after_alloc_metrics.texture_bytes, initial_metrics.texture_bytes + 1_048_576);
        }
