...
use wgpu::{Device, Queue, Buffer, BufferDescriptor, BufferUsages, CommandEncoder, 
           BindGroup, BindGroupEntry, ComputePipeline,
...
impl FeedbackBuffer {
    ...
    pub fn init_compute_pipeline(
        ...
        let shader = device.create_shader_module(ShaderModuleDescriptor {
            ...
            source: ShaderSource::Wgsl(include_str!("../shaders/virtual_texture_feedback.wgsl").into()),
        });
    ...
    fn parse_feedback_data(&self, data: &[u8]) -> Vec<TileId> {
        ...
        for chunk in data.chunks_exact(entry_size) {
            if chunk.len() == entry_size {
                ...
                let entry: FeedbackEntry = unsafe {
                    std::ptr::read(chunk.as_ptr() as *const FeedbackEntry)
                };
...
impl FeedbackManager {
    ...
    pub fn initialize(&mut self, device: &Device, page_table_buffer: &Buffer) -> Result<(), String> {
