...
pub use types::{
    ResourceHandle, PassHandle, ResourceDesc, ResourceType, PassDesc, PassType,
    ResourceInfo, PassInfo, FrameGraphMetrics, ResourceBarrier
};
...
impl FrameGraph {
    ...
    pub fn add_pass<F>(&mut self, name: &str, pass_type: PassType, setup: F) -> RenderResult<PassHandle>
    where
    ...
    pub fn get_execution_plan(&mut self) -> RenderResult<(Vec<PassHandle>, Vec<ResourceBarrier>)> {
        ...
        let pass_infos: Vec<_> = sorted_passes.iter()
            .filter_map(|&handle| self.passes.get(&handle))
            .cloned()
            .collect();
            
        // Plan barriers
        let barriers = self.barrier_planner.plan_barriers(&pass_infos, &self.resources);
    ...
    fn has_dependency(&self, pass_a: PassHandle, pass_b: PassHandle) -> RenderResult<bool> {
        let info_a = self.passes.get(&pass_a)
            .ok_or_else(|| RenderError::render("Invalid pass handle A"))?;
        let info_b = self.passes.get(&pass_b)
    ...
    fn alias_transient_resources(&mut self) -> RenderResult<()> {
        ...
        for &resource_a in &resource_handles {
            for &resource_b in &resource_handles {
                if resource_a >= resource_b { continue; } // Avoid double-checking
                ...
                if self.can_alias_resources(resource_a, resource_b)? {
                    ...
                    if let Some(info_b) = self.resources.get_mut(&resource_b) {
                        ...
                        if let Some(size) = info_b.desc.size {
                            ...
                            memory_saved += (extent.width * extent.height * extent.depth_or_array_layers * 4) as u64;
                        }
                    }
    ...
    fn can_alias_resources(&self, resource_a: ResourceHandle, resource_b: ResourceHandle) -> RenderResult<bool> {
        let info_a = self.resources.get(&resource_a)
            .ok_or_else(|| RenderError::render("Invalid resource handle A"))?;
        let info_b = self.resources.get(&resource_b)
    ...
    fn update_metrics(&mut self) {
        ...
        self.metrics.transient_count = self.resources.values()
