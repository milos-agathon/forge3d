...
fn compute_marching_squares_case(h00: f32, h10: f32, h01: f32, h11: f32, level: f32) -> u8 {
    ...
    if h00 >= level { case |= 1; }  // Bottom-left
    if h10 >= level { case |= 2; }  // Bottom-right
    if h01 >= level { case |= 4; }  // Top-left  
    if h11 >= level { case |= 8; }  // Top-right
...
fn get_marching_squares_segments(
    case: u8,
    h00: f32, h10: f32, h01: f32, h11: f32,
    level: f32,
    x0: f32, y0: f32,
    dx: f32, dy: f32
) -> Vec<((f32, f32), (f32, f32))> {
...
mod tests {
    ...
    fn test_slope_aspect_flat_surface() {
        ...
        for sa in result.iter() {
            assert!(sa.slope_deg < 0.1, "Slope should be near zero for flat surface, got {}", sa.slope_deg);
        }
    }
    ...
    fn test_slope_aspect_simple_ramp() {
        ...
        assert!(center.slope_deg > 30.0, "Ramp should have significant slope");
        
        // Aspect should be roughly eastward (around 90 degrees)
        // Note: exact values depend on finite difference implementation
        assert!(center.aspect_deg > 45.0 && center.aspect_deg < 135.0, 
               "East-facing ramp should have eastward aspect, got {}", center.aspect_deg);
    }
    ...
    fn test_contour_extraction_basic() {
        ...
        for polyline in &result.polylines {
            assert!(levels.contains(&polyline.level), "Polyline level should be one of the requested levels");
        }
    }
    ...
    fn test_contour_extraction_no_crossings() {
        ...
        assert_eq!(result.polyline_count, 0, "Flat surface should produce no contours");
    }
