...
impl LodConfig {
    pub fn new(pixel_error_budget: f32, viewport_width: u32, viewport_height: u32, fov_y: f32) -> Self {
        Self {
            pixel_error_budget,
            viewport_width,
            viewport_height,
...
pub fn select_lod_for_tile(
    ...
    TileId::new(max_lod, base_tile_id.x >> max_lod, base_tile_id.y >> max_lod)
}
...
pub fn calculate_triangle_reduction(
    ...
    let full_res_triangles: u32 = tiles_full_res.iter()
        .map(|_| base_triangles_per_tile)
        .sum();
    
    let lod_triangles: u32 = tiles_with_lod.iter()
        .map(|tile| {
...
mod tests {
    ...
    fn test_lod_selection() {
        ...
        let lod_tile = select_lod_for_tile(&bounds, base_tile, camera_pos, view, proj, &strict_config, 3);
    ...
    fn test_triangle_reduction_meets_40_percent() {
        ...
        assert!(reduction >= 0.4, "Triangle reduction {} should be >= 40%", reduction);
    }
}
