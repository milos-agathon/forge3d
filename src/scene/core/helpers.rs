fn create_grid_buffers(
    device: &wgpu::Device,
    grid: u32,
) -> Result<(TrackedBuffer, TrackedBuffer, u32), crate::core::error::RenderError> {
...
fn create_dummy_height_texture(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
) -> Result<(wgpu::TextureView, wgpu::Sampler), crate::core::error::RenderError> {
    let w = 2u32;
    let h = 2u32;
...
fn create_dummy_cloud_shadow_bind_group(
    ...
    device: &wgpu::Device,
) -> Result<wgpu::BindGroup, crate::core::error::RenderError> {
    let dummy_cloud_texture = tracked_create_texture(
        device,
        &wgpu::TextureDescriptor {
    ...
    Ok(tp.make_bg_cloud_shadows(device, &dummy_cloud_view, &dummy_cloud_sampler))
}
