...
fn recompute_normals(mesh: &mut MeshBuffers) {
    ...
    for (i, n) in mesh.normals.iter_mut().enumerate() {
        ...
        let nx = n[0] / k; let ny = n[1] / k; let nz = n[2] / k;
        let len = (nx*nx + ny*ny + nz*nz).sqrt();
        *n = if len > 0.0 { [nx/len, ny/len, nz/len] } else { [0.0, 0.0, 1.0] };
    }
}
    ...
    for _ in 0..levels {
        ...
        for tri in mesh.indices.chunks_exact(3) {
            ...
            *edge_use_count.entry(e20).or_insert(0) += 1;
            neighbors[v0 as usize].insert(v1); neighbors[v0 as usize].insert(v2);
            neighbors[v1 as usize].insert(v0); neighbors[v1 as usize].insert(v2);
            neighbors[v2 as usize].insert(v0); neighbors[v2 as usize].insert(v1);
        }
        ...
        let mut smoothed_uv = if has_uv { vec![[0.0f32;2]; old_vertex_count] } else { Vec::new() };
        ...
        for vi in 0..old_vertex_count {
            ...
            if preserve_boundary && crease_n >= 2 {
                ...
                for &nb in nbrs.iter() {
                    if crease_set.contains(&edge_key(vi as u32, nb)) || boundary_edges.contains(&edge_key(vi as u32, nb)) {
                        candidates.push(nb);
                ...
                smoothed[vi] = v;
                if has_uv { smoothed_uv[vi] = mesh.uvs[vi]; }
            }
        }
        ...
        for vi in 0..old_vertex_count {
            ...
            if has_uv { new_uvs[vi] = smoothed_uv[vi]; }
        }

        mesh.positions = new_positions;
        mesh.indices = new_indices;
        if has_uv { mesh.uvs = new_uvs; }
...
fn compute_max_edge_length(mesh: &MeshBuffers) -> f32 {
    ...
    for tri in mesh.indices.chunks_exact(3) {
        ...
        let e01 = ((p1[0]-p0[0]).powi(2) + (p1[1]-p0[1]).powi(2) + (p1[2]-p0[2]).powi(2)).sqrt();
        let e12 = ((p2[0]-p1[0]).powi(2) + (p2[1]-p1[1]).powi(2) + (p2[2]-p1[2]).powi(2)).sqrt();
        let e20 = ((p0[0]-p2[0]).powi(2) + (p0[1]-p2[1]).powi(2) + (p0[2]-p2[2]).powi(2)).sqrt();
...
fn max_dihedral_angle(mesh: &MeshBuffers) -> f32 {
    ...
    for tri in mesh.indices.chunks_exact(3) {
        let v0 = tri[0] as usize; let v1 = tri[1] as usize; let v2 = tri[2] as usize;
    ...
    if let Some(curv_th) = curvature_threshold {
        if curv_th > 0.0 {
            ...
            if max_d > curv_th {
                ...
                let l = if ratio > 1.0 { (ratio as u32).saturating_sub(1) } else { 0 };
                levels = levels.max(l);
            }
        }
    }
    if levels == 0 { return input.clone(); }
