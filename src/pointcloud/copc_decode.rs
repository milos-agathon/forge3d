    ...
    for i in 0..point_count as usize {
        ...
        if let Some(ref mut cols) = colors {
            ...
            if rgb_off + 6 <= data.len() {
                cols.push(
                    (u16::from_le_bytes([data[rgb_off], data[rgb_off + 1]]) >> 8) as u8,
                );
                cols.push(
                    (u16::from_le_bytes([data[rgb_off + 2], data[rgb_off + 3]]) >> 8) as u8,
                );
                cols.push(
                    (u16::from_le_bytes([data[rgb_off + 4], data[rgb_off + 5]]) >> 8) as u8,
                );
            }
        }
    }
...
fn decompress_laz_chunk(
    ...
    let vlr_bytes = laz_vlr_data.as_ref().ok_or_else(|| {
        PointCloudError::InvalidLaz(
            "Data is LAZ-compressed but no laszip VLR found in file".into(),
        )
    })?;

    let laz_vlr =
        laz::LazVlr::read_from(std::io::Cursor::new(vlr_bytes)).map_err(|e| {
            PointCloudError::InvalidLaz(format!("Failed to parse LAZ VLR: {}", e))
        })?;
    ...
    let mut decompressor =
        laz::LasZipDecompressor::new(cursor, laz_vlr).map_err(|e| {
            PointCloudError::InvalidLaz(format!(
                "Failed to create LAZ decompressor: {}",
                e
            ))
        })?;
    ...
    decompressor
        ...
        .map_err(|e| {
            PointCloudError::InvalidLaz(format!("LAZ decompression failed: {}", e))
        })?;
