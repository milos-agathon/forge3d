...
mod tests {
    ...
    #[test]
    fn epsg_g7_2_worked_example_inverse_at_published_precision() {
        let (lon, lat, h) =
            wgs84_ecef_to_geodetic(DVec3::new(3_771_793.968, 140_253.342, 5_124_304.349)).unwrap();
        // Published to 0.001" (≈ 0.03 m) and 0.1 m in height.
        assert!(
            (lat - dms(53.0, 48.0, 33.820)).abs() < 0.0005 / 3600.0,
            "lat residual {lat}"
        );
        assert!(
            (lon - dms(2.0, 7.0, 46.380)).abs() < 0.0005 / 3600.0,
            "lon residual {lon}"
        );
        assert!((h - 73.0).abs() < 0.05, "height residual {h}");
    }
