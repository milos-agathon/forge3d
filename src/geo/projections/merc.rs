...
mod tests {
    ...
    fn epsg_g7_2_worked_example_forward() {
        // GN7-2 prints to 0.01 m and uses 8-decimal-rounded radians; 1 cm is
        // the reference's own precision (measured residual ≈ 3.3 mm).
        let (e, n) = neiez().forward(120.0, dms(-3.0, 0.0, 0.0)).unwrap();
        assert!(
            (e - 5_009_726.58).abs() < 1e-2,
            "easting residual {} m",
            (e - 5_009_726.58).abs()
        );
        assert!(
            (n - 569_150.82).abs() < 1e-2,
            "northing residual {} m",
            (n - 569_150.82).abs()
        );
    }
    ...
    fn epsg_g7_2_worked_example_inverse() {
        let (lon, lat) = neiez().inverse(5_009_726.58, 569_150.82).unwrap();
        assert!(
            (lon - 120.0).abs() < 2e-7,
            "lon residual {} deg",
            (lon - 120.0).abs()
    ...
    fn epsg_g7_2_web_mercator_worked_example_within_1mm() {
        ...
        assert!(
            (e + 11_169_055.58).abs() < 1e-2,
            "easting residual {} m",
            (e + 11_169_055.58).abs()
        );
        assert!(
            (n - 2_800_000.00).abs() < 1e-2,
            "northing residual {} m",
            (n - 2_800_000.00).abs()
        );
        // Reverse worked example: E = -11169055.58, N = 2810000.00
        // → φ = 24°27'48.889"N, λ = 100°20'00.000"W.
        let (lon2, lat2) = web_mercator_inverse(-11_169_055.58, 2_810_000.00).unwrap();
        assert!((lon2 - lon).abs() < 1e-7, "lon residual {lon2}");
        assert!(
            (lat2 - dms(24.0, 27.0, 48.889)).abs() < 1e-7,
            "lat residual {lat2}"
        );
    }
