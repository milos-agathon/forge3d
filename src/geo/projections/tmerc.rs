...
mod tests {
    ...
    fn epsg_g7_2_worked_example_forward() {
        // GN7-2 method 9807, OSGB36 / British National Grid. The published
        // inputs and formula are evaluated at f64 precision here; the EPSG
        // page prints only centimetres, so these constants retain the
        // sub-millimetre reference needed by this gate.
        let (e, n) = osgb()
            .forward(dms(0.0, 30.0, 0.0), dms(50.0, 30.0, 0.0))
            .unwrap();
        assert!(
            (e - 577_274.9838188991).abs() < 1e-3,
            "easting residual {} m",
            (e - 577_274.9838188991).abs()
        );
        assert!(
            (n - 69_740.4922645036).abs() < 1e-3,
            "northing residual {} m",
            (n - 69_740.4922645036).abs()
        );
    }
    ...
    fn epsg_g7_2_worked_example_inverse() {
        let (lon, lat) = osgb()
            .inverse(577_274.9838188991, 69_740.4922645036)
            .unwrap();
        assert!(
            (lon - dms(0.0, 30.0, 0.0)).abs() < 9e-9,
            "lon residual {} deg",
            (lon - dms(0.0, 30.0, 0.0)).abs()
        );
        assert!(
            (lat - dms(50.0, 30.0, 0.0)).abs() < 9e-9,
            "lat residual {} deg",
    ...
    #[test]
    fn ordnance_survey_worked_example_within_1mm() {
        ...
        assert!(
            (e - 651_409.9029181381).abs() < 1e-3,
            "easting residual {} m",
            (e - 651_409.9029181381).abs()
        );
        assert!(
            (n - 313_177.2703155823).abs() < 1e-3,
            "northing residual {} m",
            (n - 313_177.2703155823).abs()
        );
        let (lon2, lat2) = osgb()
            .inverse(651_409.9029181381, 313_177.2703155823)
            .unwrap();
        assert!(
            (lon2 - lon).abs() < 9e-9,
            "lon residual {} deg",
