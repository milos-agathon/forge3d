...
mod tests {
    ...
    fn epsg_g7_2_worked_example_forward() {
        ...
        assert!(
            (e - 3_320_416.747359853).abs() < 1e-3,
            "easting residual {} m",
            (e - 3_320_416.747359853).abs()
        );
        assert!(
            (n - 632_668.4312721284).abs() < 1e-3,
            "northing residual {} m",
            (n - 632_668.4312721284).abs()
        );
    }

    #[test]
    fn epsg_g7_2_worked_example_inverse() {
        let (lon, lat) = ups_north()
            .inverse(3_320_416.747359853, 632_668.4312721284)
            .unwrap();
        assert!(
            (lon - 44.0).abs() < 9e-9,
            "lon residual {} deg",
