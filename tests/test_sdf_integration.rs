...
fn test_sdf_scene_to_hybrid_scene_conversion() {
    ...
    let (builder, box_idx) = builder
        .add_box(Vec3::new(2.0, 0.0, 0.0), Vec3::new(0.8, 0.8, 0.8), 2);

    let (builder, _union_idx) = builder
    ...
    assert!(hybrid_scene.has_sdf(), "Hybrid scene should have SDF geometry");
    assert!(!hybrid_scene.has_mesh(), "Hybrid scene should not have mesh geometry");
...
fn test_hybrid_scene_raymarching_integration() {
    ...
    assert!(result.t > 3.0 && result.t < 5.0, "Hit distance should be reasonable");
    ...
    assert!(distance_to_expected < 0.1, "Hit point should be approximately correct");
...
fn test_multiple_primitive_scene() {
    ...
    let (builder, sphere_idx) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-2.0, 0.0, 0.0), 1.0, 1);

    let (builder, box_idx) = builder
        .add_box(Vec3::new(0.0, 0.0, 0.0), Vec3::new(1.0, 1.0, 1.0), 2);

    let (builder, cylinder_idx) = builder
        .add_cylinder(Vec3::new(2.0, 0.0, 0.0), 0.8, 2.0, 3);

    let (builder, union1_idx) = builder
        .union(sphere_idx, box_idx, 4);

    let (builder, _final_union_idx) = builder
        .union(union1_idx, cylinder_idx, 0);
...
fn test_csg_operations_integration() {
    ...
    let (builder, sphere1_idx) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-0.5, 0.0, 0.0), 1.0, 1);

    let (builder, sphere2_idx) = builder
        .add_sphere(Vec3::new(0.5, 0.0, 0.0), 1.0, 2);

    let (builder, intersection_idx) = builder
        .intersect(sphere1_idx, sphere2_idx, 3);
...
fn test_smooth_csg_operations() {
    ...
    let (builder_hard, s1) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-0.3, 0.0, 0.0), 0.8, 1);

    let (builder_hard, s2) = builder_hard
        .add_sphere(Vec3::new(0.3, 0.0, 0.0), 0.8, 2);

    let (builder_hard, _hard_union) = builder_hard
        .union(s1, s2, 0);

    let hard_scene = builder_hard.build();

    // Create smooth version
    let (builder_smooth, s1) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-0.3, 0.0, 0.0), 0.8, 1);

    let (builder_smooth, s2) = builder_smooth
        .add_sphere(Vec3::new(0.3, 0.0, 0.0), 0.8, 2);

    let (builder_smooth, _smooth_union) = builder_smooth
        .smooth_union(s1, s2, 0.2, 0);
    ...
    assert!(hard_result.distance < 0.0, "Hard union should be inside at center");
    assert!(smooth_result.distance < 0.0, "Smooth union should be inside at center");
}
...
fn test_performance_characteristics() {
    ...
    assert!(elapsed.as_millis() < 1000, "Should complete in reasonable time");
...
fn test_normal_calculation_accuracy() {
    ...
    for (origin, direction) in test_rays {
        ...
        assert!((normal_length - 1.0).abs() < 0.01, "Normal should be unit length");
...
fn test_material_id_propagation() {
    ...
    let (builder, sphere_idx) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-1.0, 0.0, 0.0), 0.8, 42); // Unique material ID

    let (builder, box_idx) = builder
        .add_box(Vec3::new(1.0, 0.0, 0.0), Vec3::new(0.8, 0.8, 0.8), 99); // Different material ID

    let (builder, _union_idx) = builder
        .union(sphere_idx, box_idx, 123); // Union with its own material ID
    ...
    if result_sphere.hit {
        ...
        assert!(result_sphere.material_id == 42 || result_sphere.material_id == 123,
    ...
    if result_box.hit {
        ...
        assert!(result_box.material_id == 99 || result_box.material_id == 123,
...
mod benchmarks {
    ...
    fn bench_single_sphere_raymarching() {
        ...
        assert!(ns_per_ray < 1_000_000, "Raymarching should be reasonably fast");
    }

    #[test]
    fn bench_complex_scene_raymarching() {
        // Create a more complex scene
        let (builder, s1) = SdfSceneBuilder::new()
            .add_sphere(Vec3::new(-1.0, 0.0, 0.0), 0.8, 1);

        let (builder, s2) = builder
            .add_sphere(Vec3::new(1.0, 0.0, 0.0), 0.8, 2);

        let (builder, b1) = builder
            .add_box(Vec3::ZERO, Vec3::new(1.5, 0.3, 0.3), 3);

        let (builder, union_spheres) = builder
            .smooth_union(s1, s2, 0.2, 4);

        let (builder, _final) = builder
            .subtract(union_spheres, b1, 0);
        ...
        assert!(us_per_ray < 100, "Complex scene raymarching should complete in reasonable time");
    }
}