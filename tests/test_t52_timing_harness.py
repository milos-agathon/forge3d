...
def test_cpu_only_png_bench_smoke(op):
    res = vf.run_benchmark(op, 64, 64, iterations=3, warmup=1, seed=1)
    _check_schema(res)
    assert res["op"] == op
...
def test_gpu_bench_smoke(op):
    res = vf.run_benchmark(op, 64, 64, iterations=3, warmup=1)
    _check_schema(res)
    assert res["op"] == op
...
def test_renderer_png_bench_smoke():
    res = vf.run_benchmark("renderer_png", 64, 64, iterations=2, warmup=1)
    _check_schema(res)