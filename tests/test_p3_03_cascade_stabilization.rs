...
fn test_stabilization_consistency() {
    ...
    renderer1.update_cascades(camera_view, camera_projection, light_direction, 0.1, 100.0);

    let mut renderer2 = CsmRenderer::new(&device, config);
    renderer2.update_cascades(camera_view, camera_projection, light_direction, 0.1, 100.0);
...
fn test_stabilization_minor_camera_movement() {
    ...
    // First position
    let camera_view1 = Mat4::look_at_rh(Vec3::new(5.0, 5.0, 5.0), Vec3::ZERO, Vec3::Y);

    let mut renderer1 = CsmRenderer::new(&device, config.clone());
    renderer1.update_cascades(camera_view1, camera_projection, light_direction, 0.1, 100.0);
    ...
    let mut renderer2 = CsmRenderer::new(&device, config);
    renderer2.update_cascades(camera_view2, camera_projection, light_direction, 0.1, 100.0);
...
fn test_texel_size_calculation() {
    ...
    let test_cases = vec![(512, 3), (1024, 3), (2048, 4)];
