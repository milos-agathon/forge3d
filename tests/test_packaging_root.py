...
def test_root_import_version_and_symbols():
    assert hasattr(vf, "__version__") and isinstance(vf.__version__, str)
    for name in [
        "Renderer", "Scene",
        ...
        assert hasattr(vf, name), f"missing symbol at root: {name}"

def test_numpy_to_png_accepts_pathlike(tmp_path):
    p = tmp_path / "gray.png"
    g = np.array([[0, 127, 255],[10, 20, 30]], dtype=np.uint8)
    vf.numpy_to_png(p, g)           # PathLike write
    a = vf.png_to_numpy(p)          # PathLike read
...
def test_curated_all_does_not_leak_module_object():
    ...
    assert "_vulkan_forge" not in vf.__all__
    ...
    assert isinstance(getattr(vf, "__all__", []), list)
    assert "Renderer" in vf.__all__ and "Scene" in vf.__all__