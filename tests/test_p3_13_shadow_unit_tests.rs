...
fn test_memory_estimator_pcf_no_moments() {
    ...
    assert_eq!(
        actual, expected,
        "PCF memory should only include depth atlas"
    );
}
...
fn test_memory_estimator_vsm_with_moments() {
    ...
    assert_eq!(
        actual, expected,
        "VSM memory should include depth + 4-channel moments"
    );
}
...
fn test_memory_estimator_evsm_with_moments() {
    ...
    assert_eq!(
        actual, expected,
        "EVSM memory should include depth + 4-channel moments"
    );
}
...
fn test_memory_estimator_msm_with_moments() {
    ...
    assert_eq!(
        actual, expected,
        "MSM memory should include depth + 4-channel moments"
    );
}
...
fn test_memory_estimator_scales_with_cascades() {
    ...
    assert_eq!(
        memory_4,
        memory_2 * 2,
        "Memory should double when cascade count doubles"
    );
}
...
fn test_memory_estimator_scales_with_resolution() {
    ...
    assert_eq!(
        memory_1024,
        memory_512 * 4,
        "Memory should quadruple when resolution doubles"
    );
}
...
fn test_pcss_radius_clamp_basic() {
    ...
    let camera_projection =
        Mat4::perspective_rh(std::f32::consts::FRAC_PI_4, 16.0 / 9.0, 0.1, 200.0);
    ...
    assert!(
        clamped_blocker > 0.0,
        "Clamped blocker radius should be positive"
    );
    assert!(
        clamped_filter > 0.0,
        "Clamped filter radius should be positive"
    );
}
...
fn test_pcss_radius_reasonable_clamp() {
    ...
    assert!(
        clamped_blocker <= 0.05,
        "Should not increase blocker radius"
    );
...
fn test_cascade_splits_monotonic() {
    ...
    for i in 0..cascade_count {
        ...
        assert!(
            cascade.near_distance < cascade.far_distance,
            "Cascade {} near ({}) should be < far ({})",
            i,
        ...
        if i + 1 < cascade_count {
            ...
            assert!(
                cascade.far_distance <= next_cascade.near_distance,
                "Cascade {} far ({}) should be <= cascade {} near ({})",
                i,
                cascade.far_distance,
                i + 1,
                next_cascade.near_distance
            );
        }
    }
}
...
fn test_cascade_splits_within_near_far() {
    ...
    assert!(
        first_cascade.near_distance >= near_plane - 0.001,
        "First cascade near ({}) should be >= near_plane ({})",
        first_cascade.near_distance,
        near_plane
    );
    ...
    assert!(
        last_cascade.far_distance <= far_plane + 0.001,
        "Last cascade far ({}) should be <= far_plane ({})",
        last_cascade.far_distance,
        far_plane
    );
}
...
fn test_cascade_splits_cover_full_range() {
    ...
    assert!(
        (coverage - expected_coverage).abs() < 1.0,
        "Cascades should cover near to far range: coverage={}, expected={}",
        coverage,
        expected_coverage
    );
}
...
fn test_cascade_texel_size_increases() {
    ...
    for i in 0..cascade_count.saturating_sub(1) {
        ...
        assert!(
            next_texel_size >= texel_size * 0.9,
...
fn test_requires_moments_hard() {
    ...
    assert!(
        manager.renderer().evsm_maps.is_none(),
        "Hard shadows should not create moment maps"
    );
}
...
fn test_requires_moments_pcf() {
    ...
    assert!(
        manager.renderer().evsm_maps.is_none(),
        "PCF should not create moment maps"
    );
}
...
fn test_requires_moments_pcss() {
    ...
    assert!(
        manager.renderer().evsm_maps.is_none(),
        "PCSS should not create moment maps"
    );
}
...
fn test_requires_moments_vsm() {
    ...
    assert!(
        manager.renderer().evsm_maps.is_some(),
        "VSM should create moment maps"
    );
}
...
fn test_requires_moments_evsm() {
    ...
    assert!(
        manager.renderer().evsm_maps.is_some(),
        "EVSM should create moment maps"
    );
}
...
fn test_no_fallback_moment_texture_for_moment_techniques() {
    ...
    for technique in techniques {
        ...
        assert!(
            manager.renderer().evsm_maps.is_some(),
            "Technique {:?} should have moment maps",
            technique
        );
    }
}
