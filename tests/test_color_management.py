...
def test_gpu_tonemap_loaders_use_common_source() -> None:
    ...
    # The PBR render pipeline assembles its WGSL in shader_sources::pbr(); that
    # assembly itself must carry the shared tonemap source.
    sources = _read(ROOT / "src" / "shader_sources.rs")
    pbr_body = sources.split("fn pbr() -> String {", 1)[1].split("\n}\n", 1)[0]
    assert common in pbr_body, "shader_sources::pbr() does not load shared tonemap WGSL"
