// tests/test_pbr_pass.rs
// Sanity check for the PBR render pass wiring
// Exists to ensure bind group layouts and pipeline creation stay in sync
// RELEVANT FILES: src/render/pbr_pass.rs, src/pipeline/pbr.rs, shaders/pbr.wgsl, shaders/shadows.wgsl
...
fn try_create_device_and_queue() -> Option<(wgpu::Device, wgpu::Queue)> {
    ...
    let descriptor = wgpu::DeviceDescriptor {
        ...
        label: Some("pbr-pass-test-device"),
    };
...
#[test]
fn pbr_render_pass_initialises_pipeline() {
    let Some((device, queue)) = try_create_device_and_queue() else {
        eprintln!("Skipping PBR render pass test due to missing GPU adapter");
        return;
    };
