...
use wgpu::{Device, Queue, Instance, Backends, DeviceDescriptor, RequestAdapterOptions, BufferUsages, BufferDescriptor};
use std::sync::Arc;
...
async fn test_fence_synchronization() {
    let (device, queue) = create_test_device().await;
    let fence_tracker = Arc::new(std::sync::Mutex::new(
        FenceTracker::new(device.clone(), queue.clone())
    ));
...
async fn test_performance_overhead() {
    ...
    assert!(elapsed < Duration::from_millis(2), 
        "CPU overhead {} ms exceeds 2ms limit", elapsed.as_millis());
...
async fn test_no_buffer_reuse_before_fence() {
    ...
    if alloc3.is_none() {
        assert!(stats.buffer_stalls > 0, "Expected buffer stall to be counted");
    }
...
async fn test_ring_wrap_around() {
    ...
    for i in 0..4 { // Go one past the number of rings to test wrap
        ring.advance(i as u64);
        let stats = ring.stats();
