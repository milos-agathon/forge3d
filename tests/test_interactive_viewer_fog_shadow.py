...
@pytest.mark.slow
def test_interactive_viewer_fog_shadow_snapshot():
    """Smoke test: run interactive_viewer with fog shadows enabled and capture a snapshot.

    This ensures that the viewer path with :fog and :fog-shadow does not
    crash and produces an image on disk. Detailed volumetric behavior is
    covered by the lower-level P6 god-rays tests.
    """
