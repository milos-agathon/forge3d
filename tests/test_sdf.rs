...
fn test_sdf_primitive_sphere() {
    ...
    assert!((distance_center + 1.0).abs() < 0.001, "Distance at center should be -radius");

    let distance_surface = primitives::cpu_eval::evaluate_primitive(Vec3::new(1.0, 0.0, 0.0), &sphere);
    assert!(distance_surface.abs() < 0.001, "Point on surface should have distance ~0");

    let distance_outside = primitives::cpu_eval::evaluate_primitive(Vec3::new(2.0, 0.0, 0.0), &sphere);
    assert!(distance_outside > 0.0, "Point outside should have positive distance");
    assert!((distance_outside - 1.0).abs() < 0.001, "Distance should be 1.0");
}
...
fn test_sdf_primitive_box() {
    ...
    let distance_corner = primitives::cpu_eval::evaluate_primitive(Vec3::new(1.0, 1.0, 1.0), &box_prim);
    assert!(distance_corner.abs() < 0.001, "Corner should be on surface");

    let distance_outside = primitives::cpu_eval::evaluate_primitive(Vec3::new(2.0, 0.0, 0.0), &box_prim);
    assert!(distance_outside > 0.0, "Point outside should have positive distance");
}
...
fn test_sdf_primitive_cylinder() {
    ...
    let distance_side = primitives::cpu_eval::evaluate_primitive(Vec3::new(1.0, 0.0, 0.0), &cylinder);
    assert!(distance_side.abs() < 0.001, "Point on side should be on surface");

    // Point on top surface
    let distance_top = primitives::cpu_eval::evaluate_primitive(Vec3::new(0.0, 1.0, 0.0), &cylinder);
    assert!(distance_top.abs() < 0.001, "Point on top should be on surface");
}
...
fn test_sdf_primitive_plane() {
    ...
    assert!(distance_on_plane.abs() < 0.001, "Origin should be on XZ plane");

    let distance_above = primitives::cpu_eval::evaluate_primitive(Vec3::new(0.0, 1.0, 0.0), &plane);
    assert!((distance_above - 1.0).abs() < 0.001, "Point above should have distance 1.0");

    let distance_below = primitives::cpu_eval::evaluate_primitive(Vec3::new(0.0, -1.0, 0.0), &plane);
    assert!((distance_below + 1.0).abs() < 0.001, "Point below should have distance -1.0");
}
...
fn test_sdf_primitive_torus() {
    ...
    assert!((distance_center - 1.5).abs() < 0.001, "Distance at center should be major_radius - minor_radius");

    // Point on torus surface
    let distance_surface = primitives::cpu_eval::evaluate_primitive(Vec3::new(2.5, 0.0, 0.0), &torus);
    assert!(distance_surface.abs() < 0.001, "Point on surface should have distance ~0");
}
...
fn test_sdf_primitive_capsule() {
    ...
    assert!((distance_center + 0.5).abs() < 0.001, "Distance at center should be -radius");

    let distance_end = primitives::cpu_eval::evaluate_primitive(Vec3::new(1.5, 0.0, 0.0), &capsule);
    assert!(distance_end.abs() < 0.001, "Point at end should be on surface");
}

#[test]
fn test_csg_operations() {
    let result_a = CsgResult { distance: 0.5, material_id: 1 };
    let result_b = CsgResult { distance: 1.0, material_id: 2 };

    // Test union
    let union_result = operations::cpu_eval::union(result_a, result_b);
    assert_eq!(union_result.distance, 0.5, "Union should take minimum distance");
    assert_eq!(union_result.material_id, 1, "Union should take material from closer surface");

    // Test intersection
    let intersection_result = operations::cpu_eval::intersection(result_a, result_b);
    assert_eq!(intersection_result.distance, 1.0, "Intersection should take maximum distance");
    assert_eq!(intersection_result.material_id, 2, "Intersection should take material from farther surface");

    // Test subtraction
    let subtraction_result = operations::cpu_eval::subtraction(result_a, result_b);
    assert_eq!(subtraction_result.distance, 0.5, "Subtraction result distance");
    assert_eq!(subtraction_result.material_id, 1, "Subtraction should keep left material");

    // Test smooth union
    let smooth_union_result = operations::cpu_eval::smooth_union(result_a, result_b, 0.1);
    assert!(smooth_union_result.distance <= 0.5, "Smooth union should be <= minimum");
    assert!(smooth_union_result.distance >= 0.4, "Smooth union should be reasonable");
}
...
fn test_csg_tree_construction() {
    ...
    let box_prim = SdfPrimitive::box_primitive(Vec3::new(0.5, 0.0, 0.0), Vec3::new(0.8, 0.8, 0.8), 2);
    ...
    assert_eq!(tree.nodes.len(), 3, "Should have 3 nodes (2 leaves + 1 operation)");
...
fn test_sdf_scene_builder() {
    ...
    assert!(result_sphere.distance < 0.0, "Point should be inside sphere");
    ...
    assert!(result_outside.distance > 0.0, "Point should be outside union");
}
...
fn test_sdf_scene_with_bounds() {
    ...
    assert!(scene.in_bounds(Vec3::new(1.5, 1.5, 1.5)), "Point should be in bounds");
    assert!(!scene.in_bounds(Vec3::new(3.0, 0.0, 0.0)), "Point should be out of bounds");
}
...
fn test_hybrid_sdf_raymarching() {
    ...
    assert!(result.t > 3.9 && result.t < 4.1, "Hit distance should be approximately 4.0");
    assert!(result.sdf_distance.is_some(), "SDF hit should have SDF distance");
}

#[test]
fn test_complex_csg_scene() {
    let (builder, sphere1) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-0.5, 0.0, 0.0), 0.8, 1);

    let (builder, sphere2) = builder
        .add_sphere(Vec3::new(0.5, 0.0, 0.0), 0.8, 2);

    let (builder, box1) = builder
        .add_box(Vec3::ZERO, Vec3::new(1.5, 0.3, 0.3), 3);

    // Create intersection of the two spheres
    let (builder, intersection) = builder
        .intersect(sphere1, sphere2, 4);

    // Subtract the box from the intersection
    let (builder, _result) = builder
        .subtract(intersection, box1, 0);

    let scene = builder.build();

    // Test that the scene was built correctly
    assert_eq!(scene.primitive_count(), 3, "Should have 3 primitives");
    assert_eq!(scene.node_count(), 6, "Should have 6 nodes (3 leaves + 3 operations)");
    ...
    assert!(center_result.distance > 0.0, "Center should be outside due to subtraction");

    // Point in intersection but outside box should be inside
    let intersection_result = scene.evaluate(Vec3::new(0.0, 0.6, 0.0));
    assert!(intersection_result.distance < 0.0, "Point in intersection should be inside");
}

#[test]
fn test_smooth_csg_operations() {
    let (builder, sphere1) = SdfSceneBuilder::new()
        .add_sphere(Vec3::new(-0.5, 0.0, 0.0), 1.0, 1);

    let (builder, sphere2) = builder
        .add_sphere(Vec3::new(0.5, 0.0, 0.0), 1.0, 2);

    // Create smooth union
    let (builder, _smooth_union) = builder
        .smooth_union(sphere1, sphere2, 0.2, 0);
    ...
    assert!(result_center.distance < 0.0, "Center should be inside smooth union");
    assert!(result_midpoint.distance < 0.0, "Midpoint should be inside smooth union");
}
...
fn test_performance_metrics() {
    ...
    assert!(overhead >= 0.0, "Performance overhead should be non-negative");
