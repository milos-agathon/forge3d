...
def test_numpy_to_png_rejects_wrong_dtype(tmp_path):
    ...
    with pytest.raises(RuntimeError) as e:
        f3d.numpy_to_png(str(p), arr)  # str path still works
...
def test_helpers_accept_pathlike(tmp_path):
    ...
    f3d.numpy_to_png(p, rgb)             # pass Path object directly
    out = f3d.png_to_numpy(pathlib.Path(p))
