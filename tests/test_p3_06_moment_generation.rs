...
fn test_moment_memory_allocation() {
    ...
    assert!(vsm_memory > pcf_memory, "VSM should use more memory than PCF");
    assert!(evsm_memory > pcf_memory, "EVSM should use more memory than PCF");
    
    // VSM and EVSM should use same memory (both use RGBA32Float moments)
    assert_eq!(vsm_memory, evsm_memory, "VSM and EVSM should use same memory");
