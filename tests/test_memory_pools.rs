...
use forge3d::core::memory_tracker::{MemoryPoolManager, PoolBlock, DefragStats, MemoryPoolStats};
use wgpu::{Device, Queue, Instance, Backends, DeviceDescriptor, RequestAdapterOptions};
use std::sync::Arc;
...
async fn test_size_bucket_selection() {
    ...
    for (requested_size, expected_bucket) in test_cases.iter() {
        ...
        assert_eq!(block.size, *expected_bucket as u64, 
                   "Size {} should allocate from {} bucket", requested_size, expected_bucket);
    }
...
async fn test_fragmentation_measurement() {
    ...
    let blocks: Vec<_> = (0..10).map(|_| {
        pool_manager.allocate_bucket(1024).unwrap()
    }).collect();
...
async fn test_defragmentation() {
    ...
    let blocks: Vec<_> = (0..20).map(|_| {
        pool_manager.allocate_bucket(2048).unwrap()
    }).collect();
    ...
    println!("Defragmentation: {:.2}% -> {:.2}%, moved {} blocks, {:.2}ms", 
             frag_before * 100.0, frag_after * 100.0, 
             defrag_stats.blocks_moved, defrag_stats.time_ms);
}
...
async fn test_allocation_performance() {
    ...
    println!("Performance: {} allocations in {:.2}ms ({:.1} μs/alloc)", 
             num_allocations, allocation_time.as_secs_f64() * 1000.0,
             allocation_time.as_secs_f64() * 1000000.0 / num_allocations as f64);
    
    println!("Deallocation: {} blocks in {:.2}ms ({:.1} μs/dealloc)",
             num_allocations, deallocation_time.as_secs_f64() * 1000.0,
    ...
    assert!(avg_alloc_time_us < 50.0, "Allocation too slow: {:.1} μs", avg_alloc_time_us);
}
...
async fn test_no_leaks_reference_counting() {
    ...
    let stats_after = pool_manager.get_stats();
    assert_eq!(stats_after.active_blocks, 0, "Memory leak detected: {} blocks still active", 
...
async fn test_large_allocation_rejection() {
    ...
    assert!(result.is_err(), "Should reject allocation larger than largest bucket");
    
    let error_msg = result.unwrap_err();
    assert!(error_msg.contains("exceeds maximum bucket size"), 
            "Error should mention bucket size limit: {}", error_msg);
}
...
async fn test_pool_exhaustion_and_recovery() {
    ...
    assert!(allocation_count > 0, "Should be able to allocate at least some blocks");
    ...
    assert!(recovery_block.is_ok(), "Should recover after freeing blocks");
}
...
async fn test_largest_free_block_tracking() {
    ...
    let _blocks: Vec<_> = (0..5).map(|_| {
        pool_manager.allocate_bucket(8 * 1024 * 1024).unwrap() // 8MB blocks
    }).collect();
    
    let stats_after = pool_manager.get_stats();
    println!("Largest free block after allocations: {} bytes", stats_after.largest_free_block);
