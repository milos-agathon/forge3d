"""
AEQUITAS color conversion and CIEDE2000 scoring primitives.

This is the production copy of the canonical color-difference implementation
used by the AEQUITAS adjudication and DIFFERENTIA recovery gates.  It is pure
NumPy so the optional recovery score also works from an installed wheel,
without importing the repository's test package or an external color-science
dependency.

sRGB decode uses the piecewise IEC 61966-2-1 curves; XYZ uses the D65 Rec.709
primaries; CIELAB uses the D65 reference white.  The dE2000 implementation
...
def median_delta_e2000_linear(a: np.ndarray, b: np.ndarray) -> float:
    """Median per-texel ΔE2000 between two equal-shaped linear-RGB maps.

    This is the DIFFERENTIA albedo-recovery score.  Inputs are already-linear
    RGB reflectance values; the conversion therefore skips sRGB decoding.
    """
...
def delta_e_2000(lab1: np.ndarray, lab2: np.ndarray) -> np.ndarray:
    ...
    Faithful to Sharma et al. (2005), including the G-based a' rescale,
    hue-difference wrap rules, the T term, and the RT rotation term.  The
    parametric factors are kL = kC = kH = 1.
    """
...
def shadow_boundary_band(
    ...
    """Boolean (H, W) band around the lit/shadow boundary of a reference."""
    lit = relative_luminance(reference_rgb) > lum_threshold
    interior = _binary_dilate(~lit, 1)
    boundary = lit & interior
    return _binary_dilate(boundary, band_px)


def band_bbox(band: np.ndarray) -> tuple[slice, slice]:
    """Tight bounding-box slices of a boolean band mask."""
