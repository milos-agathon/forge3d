...
class GlobeScene:
    """Native COG-backed globe scene whose waypoints remain inside the seeded overview tile.

    The constructor target selects one fully covered finite overview tile.
    ``fly_to`` and every custom scripted-descent waypoint fail before rendering
    when their longitude/latitude falls outside that tile.
    """
    def __init__(
        self,
    ...
    def fly_to(self, lon: float, lat: float, altitude: float) -> Frame:
        """Render one waypoint inside the seeded overview tile at 0..408,000 metres."""
        ...
    def scripted_descent(
        self,
        waypoints: Optional[Sequence[Tuple[float, float, float]]] = ...,
    ) -> None:
        """Render the default target descent or a prevalidated in-tile custom path."""
        ...
