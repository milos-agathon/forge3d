...
Current release: 1.9.8 — Cloud-Optimized GeoTIFF (COG) streaming with HTTP range requests, IFD parsing, and LRU caching enabling continent-scale terrain rendering directly from cloud storage without full file download.

## Latest Feature: Cloud-Native Data & COG Streaming

**forge3d** now streams terrain data directly from cloud storage using Cloud-Optimized GeoTIFF (COG) format, eliminating the need for full file downloads or pre-tiling. The P3.1–P3.3 implementation delivers HTTP range request support with `reqwest`, IFD (Image File Directory) parsing for automatic overview selection, and LRU cache with configurable memory budgets. Render continental-scale DEMs by fetching only visible tiles on-demand—range requests reduce bandwidth by downloading just the bytes you need (e.g., a single 256×256 tile instead of gigabytes). The overview pyramid integration automatically selects appropriate resolution based on viewport and LOD requirements, while speculative prefetch reduces latency on sequential access patterns. Cache hit/miss statistics expose performance metrics for tuning. Run `python examples/cog_streaming_demo.py --url https://example.com/terrain.tif` to experience remote rendering with sub-millisecond tile retrieval on cache hits. The deterministic decoder ensures reproducible results across cache states, with memory budget enforcement preventing OOM on large-scale operations. Native decoder handles standard COG layouts with rasterio fallback for comprehensive format support.
