...
| R-012 | P2 VT normal/mask requests may be accepted but ignored by the native runtime. | Feature `006` T001 confirmed the extension paths: `python/forge3d/terrain_params.py` accepts `normal`/`mask` and exposes `validate_terrain_vt_support(...)`, while `src/terrain/render_params/native_vt.rs` and `src/terrain/renderer/virtual_texture.rs` document and implement albedo-only native paging. | Feature `006` must implement native normal/mask runtime support or keep emitting deterministic pre-render `vt_unsupported_family` diagnostics through `MapScene` tests/docs. Next task group: T002/T003 diagnostic contract and adapters. | Open |
| R-013 | Textured building material support may fall back to scalar PBR without diagnostics. | Feature `006` T002/T003 added structured `missing_texture_path`, `missing_uvs`, `unsupported_texture_format`, and scalar fallback diagnostics in `python/forge3d/diagnostics.py` and MapScene building texture metadata adapters in `python/forge3d/map_scene.py`; `tests/test_p2_diagnostics_contract.py` reports `4 passed`. Real textured building fixtures, docs, and end-to-end render/diagnostic coverage remain planned. | Feature `006` must add real textured building support or complete fixture/docs coverage for diagnosed missing texture path, missing UVs, unsupported format, and fallback material behavior. Next relevant task group after VT: T008/T009/T010. | Open |
| R-014 | Large-scene stats may remain fragmented across lower-level modules. | Feature `006` T002/T003 added structured `unavailable_cache_lod_stats` and `unsupported_instancing_path` diagnostics plus MapScene metadata adapters; `tests/test_p2_diagnostics_contract.py` reports `4 passed`. A unified product `LargeSceneResourceSummary` across terrain, point cloud, buildings, tiles, cache/LOD, instancing, and bottleneck layers remains unimplemented. | Feature `006` must normalize available stats or emit diagnostics across the required layer types. Next relevant task group after VT/building/label slices: T019/T020/T021/T022. | Open |
