...
| Typography controls | `experimental` | `ViewerHandle.set_label_typography(...)` does not claim native typography mutation yet; it fails honestly with diagnostics. | `experimental_feature`. |
| Decluttering controls | `experimental` | `ViewerHandle.set_declutter_algorithm(...)` does not claim alternate placement behavior yet; it fails honestly with diagnostics. | `experimental_feature`. |
