...
4. The only world-coordinate narrowing implementation is `Anchor::to_render_f32(Coord<C, E>)` (`src/camera/anchor.rs`). New code tags positions with `SceneCoord::scene` and calls `to_render_f32`, `view_look_at`, or `model_offset`; spans and directions use `Anchor::offset_to_render(SceneOffset::scene(..))`. It does not cast a world component. (MENSURA closure removed the earlier untyped `narrow`/`to_render_vec3`/`to_render_direction` helpers; the component tables below describe the M-06 plan as written.)
