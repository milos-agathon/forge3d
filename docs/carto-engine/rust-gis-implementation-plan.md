...
Current focused evidence (worktree `mensura`, HEAD `3de8f620` or later): the 21 MENSURA/GIS/API-contract Python files (`test_api_contracts`, `test_epsg_g7_2`, `test_geodesic_karney`, `test_geoid_egm96`, `test_crs_*`, `test_gis_*`, `test_m06_anchoring_boundary`, `test_world_coord_f32_gate`, `test_p1_label_layer_crs_terrain`) report **749 passed, 3 skipped**. The 3 skips are honest guards, not missing capability: one empty API-contract parameter set, and two `backend_unavailable` overlay contracts that run *only* in a no-topology build and correctly skip here because `geos-topology` is present. (Absolute counts are set-sensitive — this is the single count in this doc; do not add a second.) `cargo fmt --check`, `cargo forge3d-clippy`, and `cargo test --doc` (all five `compile_fail` contracts) pass. The built extension measured `7.105e-14 degrees` maximum angular and `8.425e-09 m` ECEF conservation residual over 10,000 points, `0.4593 m` worst EGM96 residual, `3.725e-09 m`/`1.191e-10 degrees` worst Karney inverse residual, per-method projection round-trip maxima of `1.58e-6`–`5.54e-6 mm`, and one grep-gated world-coordinate narrowing site.

**Status honestly: M-05 is complete; M-01/M-02/M-03/M-04 are near-complete with the explicit open items marked `[~]`/`[ ]` in each section below; M-06 is PARTIAL; M-07 tracks the residual evidence/fixtures.** Per line 221, MENSURA is "complete" only when every acceptance criterion is green in the shipped wheel — that bar is not yet cleared. The absolute-geospatial production paths (offscreen `Scene`, 3D-Tiles, point clouds, CityJSON) are anchored to the single `Anchor::narrow` cliff (verified), but the interactive viewer is un-anchored: its IPC camera/label fields are `f32` and `set_look_at` applies no magnitude limit, so absolute projected coordinates pushed through it lose precision — its terrain-local frame is a Python-side *convention*, not a Rust contract. That is the principal remaining implementation gap (see [`mensura-m06-world-coord-anchoring.md`](./mensura-m06-world-coord-anchoring.md)). Other open items are evidence/fixtures: M-02's external per-method PROJ oracle over the full 10k corpus (dev-only) and M-03's COG/Terrarium integration fixtures.
...
**Current status: PARTIAL — production paths anchored, interactive viewer not.** A full data-flow trace shows the paths carrying *absolute geospatial* coordinates in production — offscreen `Scene`, 3D-Tiles, point clouds, CityJSON (origin-relative) — keep them in `f64` and narrow only through the single `Anchor::narrow` site (verified). But the **interactive viewer is un-anchored**: `set_look_at` (`camera_controller.rs`) stores an arbitrary `f32` orbit target with no magnitude limit (the `clamp(0.1,1000)` is on the *zoom* distance only), and the IPC camera/label fields are `f32`. Its terrain-local frame is a Python-side *convention*, not a Rust contract — absolute projected coordinates pushed through the viewer IPC lose precision. See [`mensura-m06-world-coord-anchoring.md`](./mensura-m06-world-coord-anchoring.md); the verified half is locked by `tests/test_m06_anchoring_boundary.py`.
...
- [ ] **OPEN (viewer):** anchor the interactive viewer — widen the f32 IPC camera/label/overlay world fields to f64, add a `camera_anchor` to the viewer struct, and build an anchor-relative view matrix + geometry offsets. Rigid-translation change; end-to-end validation needs a rendered frame from the running viewer (see the anchoring doc's Residual section).
- [x] Audited every GPU uniform/WGSL world-position binding: shader-facing positions are render-space `f32` (correct).
- [x] `test_world_coord_f32_gate.py`: exactly one `as f32` in `src/camera/anchor.rs`, no `Vec3::new(x as f32, y as f32, z as f32)` reconstruction. `test_m06_anchoring_boundary.py` locks the anchored production paths and the viewer's no-Earth-scale-geodesy invariant (it does NOT claim the viewer enforces a local frame — it does not).
...
- [~] `cargo fmt --check`, `cargo forge3d-clippy`, `cargo test --doc`, and the focused MENSURA + API-contract + GIS Python suites pass — see the single evidence summary at line 127 for the current count (do not restate it here; two independently-maintained counts is how this doc previously drifted). Open: the local full curated `cargo test` matrix hangs on an unrelated GPU test in this worktree (targeted `cargo test --lib` per module is green); CI carries the full matrix incl. `geos-topology`.
