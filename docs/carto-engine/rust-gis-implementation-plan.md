...
| CRS parsing and transforms: `parse_crs`, `CrsTransform`, `transform_bounds`, `web_mercator_bounds` | **MENSURA dispatch complete for the shipped methods** | Public dispatch supports same-CRS, EPSG:4326, EPSG:3857, WGS84 UTM 32601-32660/32701-32760, and — via the authoritative `epsg_projection_definition` table (M-02) that `src/gis/crs.rs` and the writer allowlist both consume — LCC 2SP (2154), AEA (5070), Polar Stereographic A (5041/5042), Mercator A (3395), and generic transverse Mercator (method dict). The 10,000-point-per-method PROJ oracle validates all of them below 1 mm. RGF93 v1 (2154) and NAD83 (5070) reach WGS 84 only through the published EPSG null transformations EPSG:1671 (1 m) and EPSG:1188 (4 m); `CrsTransform.datum_operations` names them, and `reproject_raster`/`reproject_vector`/`prepare_dem` emit one `datum_null_transformation` diagnostic per applied operation instead of treating the datums as silently equivalent. The optional `proj` implementation in `src/geo/reproject.rs` is not used by `src/gis/crs.rs` and is omitted from wheels; arbitrary WKT/PROJ transforms are still not claimed. |
...
**Current status: complete.** `RasterInfo.height_system` is a first-class field with constants + validation, preserved through resample/reproject/align (`operation_info`), emitted/ingested at the Python dict boundary, persisted across a GeoTIFF write→read round trip via a private ASCII tag (65001), and set by the Terrarium and `prepare_dem` producers (incl. ChartDatum). `prepare_dem` converts a declared `orthometric_egm96` DEM in EPSG:4326 or in any built-in projected CRS (each affine pixel centre is inverted to WGS 84 through the pure-Rust engine) and raises for any other CRS. The Earth-fixed boundary is enforced at **compile time** by `Height<Ellipsoidal>` on every geocentric entry point (`geo::projections::geocentric::{geodetic_to_ecef, wgs84_geodetic_to_ecef}` and `tiles3d::bounds::wgs84_to_ecef`; there is no runtime DEM→ECEF path). File and Python boundaries carry the vertical datum as a runtime tag because it is only known at runtime; it becomes typed at the named `Height::<S>::new` declaration points. Preservation is now proven through full/windowed read, tiled/COG read (`read_cog`), resample, reproject, align, GeoTIFF write→read, Terrarium decode, `prepare_dem`, and a domain helper (`prepare_terrain_derivatives`), with the orthometric→ellipsoidal tag change, chart_datum/unspecified non-promotion, invalid-tag ingestion rejection, and horizontal-CRS-implies-no-vertical rules all locked in `tests/test_height_system_safety.py`.
