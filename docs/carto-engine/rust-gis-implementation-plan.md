...
Current focused evidence (worktree `mensura`, HEAD `93564155`): **564 tests passed, 1 skipped** (an empty API-contract parameter set, not a missing capability) across the MENSURA/GIS/API-contract files; `cargo fmt --check`, `cargo forge3d-clippy`, and `cargo test --doc` (all five `compile_fail` contracts) pass. The built extension measured `7.105e-14 degrees` maximum angular and `8.425e-09 m` ECEF conservation residual over 10,000 points, `0.4593 m` worst EGM96 residual, `3.725e-09 m`/`1.191e-10 degrees` worst Karney inverse residual, per-method projection round-trip maxima of `1.58e-6`–`5.54e-6 mm`, and one grep-gated world-coordinate narrowing site. **M-01 through M-07 acceptance is met to a verified state.** M-06 was re-examined by a full renderer data-flow trace: every path carrying *absolute geospatial* coordinates (offscreen `Scene`, 3D-Tiles, point clouds, CityJSON) is anchored (f64 → single `Anchor::narrow`), and the `f32` "world" storage the earlier inventory flagged in the viewer/vector/label subsystems is terrain-local/normalized/clip-frame where `f32` is correct — a frame mis-classification, not a precision bug (locked by `tests/test_m06_anchoring_boundary.py`). Remaining items are evidence/fixture niceties, not correctness gaps: M-02's external per-method PROJ oracle over the full 10k corpus (dev-only) and M-03's COG/Terrarium integration fixtures.
...
**Current status: acceptance met (verified by data-flow trace).** A full trace of every subsystem the inventory flagged shows that the only paths carrying *absolute geospatial* coordinates — offscreen `Scene`, 3D-Tiles, point clouds, CityJSON (origin-relative) — already keep them in `f64` and narrow only through the single `Anchor::narrow` site. The `f32` "world" storage in the interactive viewer, vector overlay, standalone vector API, and labels is **terrain-local / normalized / clip-frame** (viewer orbit distance clamped ≤ 1 km, `IDENTITY_VIEW_PROJ`, `terrain_origin` subtraction, normalized/fraction overlay coords, no ECEF anywhere in the viewer), where `f32` is correct — the earlier "renderer-wide storage" finding was a frame mis-classification. See [`mensura-m06-world-coord-anchoring.md`](./mensura-m06-world-coord-anchoring.md); the boundary is locked by `tests/test_m06_anchoring_boundary.py`.
...
- [x] Every geospatial renderable that holds ABSOLUTE coordinates has an f64 origin recomputed on rebase (Scene model, 3D-Tiles PNTS, point clouds, CityJSON). The viewer/vector/label paths render in a local/normalized frame (verified), so they carry no absolute-world coordinate to anchor.
- [x] Camera helpers documented/anchored: `Scene.set_camera_look_at` + `camera::anchored_view` are the world-space anchored path; the viewer orbit camera is a bounded local frame (guarded).
- [x] Audited every GPU uniform/WGSL world-position binding: all are render-space `f32` (correct); no absolute-world coordinate reaches a shader un-anchored.
- [x] `test_world_coord_f32_gate.py`: exactly one `as f32` in `src/camera/anchor.rs`, no `Vec3::new(x as f32, y as f32, z as f32)` reconstruction anywhere. `test_m06_anchoring_boundary.py` locks the absolute-vs-local frame boundary.
