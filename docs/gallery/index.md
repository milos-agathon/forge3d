...
CI runs required recipe goldens in the `Visual Goldens (NVIDIA Vulkan)` job. The protected path installs `pyproj`, proves a physical NVIDIA/Vulkan adapter with `python scripts/terrain_ci_probe.py --mode terrain --require-nvidia-vulkan`, then runs terrain and recipe image comparisons with `FORGE3D_RUN_TERRAIN_GOLDENS=1`. An absent adapter, skipped test, or pixel mismatch is fatal. Metal coverage is an opt-in support diagnostic and is not acceptance evidence. `FORGE3D_ALLOW_SOFTWARE_GOLDENS=1` remains scoped to local recipe meta-validation and must never be treated as the render-quality gate.
