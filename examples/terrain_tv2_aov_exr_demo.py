...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render a real DEM from assets/tif with terrain beauty + AOV outputs."
    )
    parser.add_argument("--dem", type=Path, default=DEFAULT_DEM, help="Path to a DEM GeoTIFF")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("terrain_tv2_output"),
        help="Directory for PNG and EXR outputs",
    )
    ...
    beauty_path = output_dir / "terrain_tv2_beauty.png"
    exr_path = output_dir / "terrain_tv2_multichannel.exr"
    beauty_frame.save(str(beauty_path))
    aov_frame.save_all(str(output_dir), "terrain_tv2")
    ...
    print(f"Wrote {output_dir / 'terrain_tv2_albedo.png'}")
    print(f"Wrote {output_dir / 'terrain_tv2_normal.png'}")
    print(f"Wrote {output_dir / 'terrain_tv2_depth.png'}")
