...
def demo_basic_palette_switching():
    ...
    try:
        ...
        for palette in test_palettes:
            ...
            if current == palette:
                print(f"  ✓ Successfully switched to: {palette}")
            else:
                print(f"  ✗ Switch failed: requested {palette}, got {current}")
    ...
    try:
        ...
        for i, palette in enumerate(palettes):
            try:
                ...
                # Save image
                output_file = f"{output_dir}/terrain_{i+1:02d}_{palette}.png"
                f3d.numpy_to_png(output_file, rgba)
                
                print(f"  {i+1:2d}. {palette:15} -> {output_file}")
                
            except Exception as e:
                print(f"  {i+1:2d}. {palette:15} -> ERROR: {e}")
...
def demo_palette_performance():
    ...
    try:
        ...
        if overhead < 20:
            print("  ✓ Low overhead - efficient palette switching")
        elif overhead < 50:
            print("  ~ Moderate overhead - acceptable for most uses")
        else:
            print("  ⚠ High overhead - consider optimizations")
    ...
    try:
        ...
        for palette in palettes:
            try:
                ...
                print(f"{palette:17} | {brightness:10.3f} | {contrast:8.3f} | {dominant_channel[0]} dominant")
                
            except Exception as e:
                print(f"{palette:17} | ERROR: {str(e)[:30]}")
...
def demo_terrain_height_mapping():
    ...
    try:
        ...
        for palette in test_palettes:
            ...
            output_file = f"{output_dir}/height_mapping_{palette}.png"
            f3d.numpy_to_png(output_file, rgba)
            
            print(f"  {palette:15} -> {output_file}")
...
def demo_interactive_palette_switching():
    ...
    try:
        ...
        for i, (palette, action) in enumerate(switch_sequence):
            ...
            if switch_time < 0.1:
                print("  ✓ Responsive - good for real-time interaction")
            elif switch_time < 0.5:
                print("  ~ Acceptable - usable for interaction")
            else:
                print("  ⚠ Slow - may impact user experience")
